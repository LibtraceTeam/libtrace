...
// Something to do with fragmented packets and hashing problems !! TODO figure
// out if this needs to be on
...
struct linux_format_data_out_t {
    ...
    /* Used to determine buffer size for the ring buffer */
    uint32_t max_order;
    /* Maximum number of packets allowed in the tx queue before notifying the
     * kernel */
    int tx_max_queue;
};
 ...
 */
static inline int
linuxcommon_to_packet_fanout(libtrace_t *libtrace,
                             struct linux_per_stream_t *stream)
{
    int fanout_opt;
    int attempts = 0;
    while (attempts < 5) {
        ...
        if (setsockopt(stream->fd, SOL_PACKET, PACKET_FANOUT, &fanout_opt,
                       sizeof(fanout_opt)) == -1) {
