...
static bool pcapng_can_write(libtrace_packet_t *packet)
{
    ...
    if (ltype == TRACE_TYPE_CONTENT_INVALID || ltype == TRACE_TYPE_UNKNOWN ||
        ltype == TRACE_TYPE_ERF_META || ltype == TRACE_TYPE_NONDATA) {
...
static inline uint16_t pcapng_get_nrb_record_type(libtrace_packet_t *packet,
                                                  char *ptr)
{
...
static inline uint16_t pcapng_get_nrb_record_len(libtrace_packet_t *packet,
                                                 char *ptr)
{
    struct pcapng_nrb_record *hdr = (struct pcapng_nrb_record *)ptr;
    if (DATA(packet->trace)->byteswapped) {
...
static uint32_t pcapng_output_options(libtrace_out_t *libtrace,
                                      libtrace_packet_t *packet, char *ptr)
{
    ...
    while ((optval = pcapng_parse_next_option(
                packet->trace, &bodyptr, &optcode, &optlen,
                (pcapng_hdr_t *)packet->buffer))) {
...
static uint32_t pcapng_output_interface_packet(libtrace_out_t *libtrace,
                                               libtrace_packet_t *packet)
{
    ...
    /* If the input trace is not pcapng we have no way of finding the
     * byteordering this can occur if a packet is reconstructed with a
     * deadtrace. Or if the packet is in the correct byte ordering just output
     * it */
    if ((packet->trace->format->type != TRACE_FORMAT_PCAPNG) ||
...
static uint32_t pcapng_output_simple_packet(libtrace_out_t *libtrace,
                                            libtrace_packet_t *packet)
{
...
static uint32_t pcapng_output_old_packet(libtrace_out_t *libtrace,
                                         libtrace_packet_t *packet)
{
...
static uint32_t pcapng_output_nameresolution_packet(libtrace_out_t *libtrace,
                                                    libtrace_packet_t *packet)
{
    ...
    while (record_type != PCAPNG_NRB_RECORD_END) {
        ...
        /* calculate any required padding. record also contains the 8 byte
         * header but we dont need to subtract it because it will be removed
         * with % 4 */
        padding = recordlen % 4;
        if (padding) {
            padding = 4 - padding;
...
static uint32_t pcapng_output_custom_packet(libtrace_out_t *libtrace,
                                            libtrace_packet_t *packet)
{
...
static uint32_t pcapng_output_enhanced_packet(libtrace_out_t *libtrace,
                                              libtrace_packet_t *packet)
{
...
static uint32_t pcapng_output_interfacestats_packet(libtrace_out_t *libtrace,
                                                    libtrace_packet_t *packet)
{
...
static void pcapng_create_output_sectionheader_packet(libtrace_out_t *libtrace)
{
    ...
    wandio_wwrite(DATAOUT(libtrace)->file, &sechdr.blocklen,
                  sizeof(sechdr.blocklen));

    DATAOUT(libtrace)->sechdr_count += 1;
}

static void pcapng_create_output_interface_packet(libtrace_out_t *libtrace,
                                                  libtrace_linktype_t linktype)
{
    ...
    wandio_wwrite(DATAOUT(libtrace)->file, &inthdr.blocklen,
                  sizeof(inthdr.blocklen));
...
static int pcapng_init_input(libtrace_t *libtrace)
{
    ...
    DATA(libtrace)->interfaces =
        (pcapng_interface_t **)calloc(10, sizeof(pcapng_interface_t));
...
static int pcapng_config_output(libtrace_out_t *libtrace,
                                      ...
                                      pcapng_hdr_t *blockhdr)
{
    ...
    // If there is not enough space for another header we've encountered an
    // error
...
static int pcapng_get_framing_length(const libtrace_packet_t *packet)
{
    ...
    trace_set_err(packet->trace, TRACE_ERR_BAD_PACKET,
                  "Invalid RT type for pcapng packet: %u", packet->type);
    return -1;
}
                                 ...
                                 libtrace_rt_types_t rt_type, uint32_t flags)
{
    ...
    if (hdrlen < 0) {
        trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                      "X Invalid RT type for pcapng packet: %u", packet->type);
        return -1;
    }
...
static int pcapng_write_packet(libtrace_out_t *libtrace,
                               libtrace_packet_t *packet)
{
    ...
    if (!DATAOUT(libtrace)->file) {
        DATAOUT(libtrace)->file = trace_open_file_out(
            libtrace, DATAOUT(libtrace)->compress_type,
            DATAOUT(libtrace)->compress_level, DATAOUT(libtrace)->flag);
    }
    ...
    case PCAPNG_SIMPLE_PACKET_TYPE: {
        /* If no section header or interface packets have been received create
         * and output them. This can occur when discard meta is enabled and the
         * input format is also pcapng */
    ...
    /* If we get this far the packet is not a pcapng type so we need to
     * encapsulate it within a enhanced pcapng packet */
    uint32_t remaining;
    void *link;
    uint32_t blocklen;
    uint32_t padding;
    ...
    epkthdr.interfaceid =
        pcapng_swap32(libtrace, DATAOUT(libtrace)->nextintid - 1);
    ...
    wandio_wwrite(DATAOUT(libtrace)->file, &epkthdr.blocklen,
                  sizeof(epkthdr.blocklen));
...
static int pcapng_read_section(libtrace_t *libtrace, libtrace_packet_t *packet,
                               uint32_t flags)
{
    ...
    if (sechdr->blocktype != PCAPNG_SECTION_TYPE) {
        trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                      "Invalid block type in pcapng section block");
        return -1;
    }
    ...
    if (DATA(libtrace)->byteswapped) {
        if (byteswap16(sechdr->majorversion) != 1 &&
            byteswap16(sechdr->minorversion) != 0) {
            trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
    ...
    if (to_read > LIBTRACE_PACKET_BUFSIZE) {
        trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                      "Excessively large section header contents of %u bytes, "
                      "likely a corrupt trace.",
                      to_read);
        return -1;
    }
    ...
    if (pcapng_prepare_packet(libtrace, packet, packet->buffer, packet->type,
                              flags)) {
        return -1;
    }
...
static int pcapng_read_interface(libtrace_t *libtrace,
                                 libtrace_packet_t *packet, uint32_t blocklen,
                                 uint32_t flags)
{
    ...
    if (DATA(libtrace)->byteswapped) {
        if (byteswap32(inthdr->blocktype) != PCAPNG_INTERFACE_TYPE) {
            trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                          "Invalid block type in pcapng interface");
            return -1;
        }
    ...
    if (DATA(libtrace)->nextintid == DATA(libtrace)->allocatedinterfaces) {
        ...
        DATA(libtrace)->interfaces = (pcapng_interface_t **)realloc(
            DATA(libtrace)->interfaces,
            DATA(libtrace)->allocatedinterfaces * sizeof(pcapng_interface_t *));
        memset(&DATA(libtrace)->interfaces[DATA(libtrace)->nextintid], 0,
               sizeof(void *) * 10);
    }
    ...
    do {
        optval = pcapng_parse_next_option(libtrace, &bodyptr, &optcode, &optlen,
                                          (pcapng_hdr_t *)packet->buffer);
        if (optval == NULL) {
            trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                           ...
                           uint32_t blocklen, uint32_t flags)
{
    ...
    if (DATA(libtrace)->byteswapped) {
        if (byteswap32(hdr->blocktype) != PCAPNG_NAME_RESOLUTION_TYPE) {
            trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                          "Invalid block type in pcapng name "
                          "resolution block");
            return -1;
        }
    } else {
                             ...
                             uint32_t blocklen, uint32_t flags)
{
    ...
    if (DATA(libtrace)->byteswapped) {
        ...
        timestamp = ((uint64_t)(byteswap32(hdr->timestamp_high)) << 32) +
                    byteswap32(hdr->timestamp_low);
    } else {
        ...
        timestamp =
            ((uint64_t)(hdr->timestamp_high) << 32) + hdr->timestamp_low;
    }
    ...
    if (interface == NULL) {
        trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                      "Unknown pcapng interface id: %u", ifaceid);
        return -1;
    }
    packet->type = TRACE_RT_PCAPNG_META;
    ...
    do {
        optval =
            pcapng_parse_next_option(packet->trace, &bodyptr, &optcode, &optlen,
                                     (pcapng_hdr_t *)packet->buffer);
        if (optval == NULL) {
            trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                              ...
                              uint32_t blocklen, uint32_t flags)
{
    ...
    if (interface == NULL) {
        trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                      "Unknown pcapng interface id: %u", 0);
        return -1;
    }
...
static int pcapng_read_packet(libtrace_t *libtrace, libtrace_packet_t *packet)
{
    ...
    if (!libtrace->format_data) {
        trace_set_err(libtrace, TRACE_ERR_BAD_FORMAT,
                      "Trace has no format data in "
                      "pcapng_read_packet()");
        return -1;
    }
    if (!libtrace->io) {
        trace_set_err(libtrace, TRACE_ERR_BAD_IO,
                      "Trace has no valid file handle "
    ...
    while (!gotpacket) {
        ...
        // Warning: the byteorder might not be set yet, the section header sets
        // this
        if (DATA(libtrace)->byteswapped) {
        ...
        // Check we won't read off the end of the packet buffer. Assuming
        // corruption. Exclude the SECTION header, as this is used to identify
        // the byteorder
        ...
        if (btype != PCAPNG_SECTION_TYPE) {
            // Read the entire block, unless it is a section as our byte
            // ordering has not been set yet.
            err = pcapng_read_body(libtrace, packet->buffer, to_read);
            if (err <= 0) {
                return err;
            }
            if (*((uint32_t *)((char *)packet->buffer + to_read - 4)) !=
                peeker.blocklen) {
                trace_set_err(
                    libtrace, TRACE_ERR_BAD_PACKET,
        ...
        /* Section Header */
        case PCAPNG_SECTION_TYPE:
            /* Section header packets are required for PCAPNG so even if
             * discard_meta option is set it still needs to be processed. Not
             * setting gotpacket will prevent triggering the meta callback */
        ...
        case PCAPNG_ENHANCED_PACKET_TYPE:
            err = pcapng_read_enhanced(libtrace, packet, to_read, flags);
            gotpacket = 1;
            break;
...
pcapng_get_direction(const libtrace_packet_t *packet)
{
...
static struct timespec pcapng_get_timespec(const libtrace_packet_t *packet)
{
    ...
    if (!packet->header) {
        trace_set_err(packet->trace, TRACE_ERR_BAD_PACKET,
                      "NULL header in packet in pcapng_get_timespec()");
        /* Return default timespec on error? */
        return ts;
    }
    ...
    if (PACKET_IS_ENHANCED) {
        ...
        if (DATA(packet->trace)->byteswapped) {
            timestamp = ((uint64_t)(byteswap32(ehdr->timestamp_high)) << 32) +
                        byteswap32(ehdr->timestamp_low);
            interfaceid = byteswap32(ehdr->interfaceid);
        } else {
            timestamp =
                ((uint64_t)(ehdr->timestamp_high) << 32) + ehdr->timestamp_low;
        ...
        if (DATA(packet->trace)->byteswapped) {
            timestamp = ((uint64_t)(byteswap32(ohdr->timestamp_high)) << 32) +
                        byteswap32(ohdr->timestamp_low);
            interfaceid = byteswap16(ohdr->interfaceid);
        } else {
            timestamp =
                ((uint64_t)(ohdr->timestamp_high) << 32) + ohdr->timestamp_low;
    ...
    if (interface == NULL) {
        trace_set_err(packet->trace, TRACE_ERR_BAD_PACKET,
                      "Bad interface %u on pcapng packet", interfaceid);
        return ts;
    }
...
static inline int pcapng_get_wlen_header(const libtrace_packet_t *packet)
{
    ...
    } else if (PACKET_IS_SECTION || PACKET_IS_INTERFACE ||
               PACKET_IS_NAME_RESOLUTION || PACKET_IS_INTERFACE_STATS ||
               PACKET_IS_CUSTOM || PACKET_IS_CUSTOM_NONCOPY ||
               PACKET_IS_DECRYPTION_SECRETS) {
...
static int pcapng_get_capture_length(const libtrace_packet_t *packet)
{
    ...
    if (PACKET_IS_ENHANCED) {
         ...
         */
        if (DATA(packet->trace)->byteswapped) {
            return byteswap32(shdr->blocklen) - sizeof(pcapng_spkt_t) - 4;
        } else {
...
static size_t pcapng_set_capture_length(libtrace_packet_t *packet, size_t size)
{
    uint32_t current;
    ...
    if (PACKET_IS_SIMPLE) {
        ...
        if (DATA(packet->trace)->byteswapped) {
            tocopy =
                byteswap32(ehdr->blocklen) - sizeof(pcapng_epkt_t) - current;
        } else {
            tocopy = ehdr->blocklen - sizeof(pcapng_epkt_t) - current;
        }
    }
...
static libtrace_meta_datatype_t pcapng_get_datatype(uint32_t section,
                                                    uint32_t option)
{
    switch (section) {
    case (PCAPNG_SECTION_TYPE):
    ...
    case (PCAPNG_OLD_PACKET_TYPE):
        switch (option) {
        case (PCAPNG_META_OLD_FLAGS):
            return TRACE_META_UINT32;
        case (PCAPNG_META_OLD_HASH):
            return TRACE_META_STRING;
        }
        break;
    case (PCAPNG_SIMPLE_PACKET_TYPE):
    ...
    case (PCAPNG_ENHANCED_PACKET_TYPE):
        switch (option) {
        case (PCAPNG_META_EPB_FLAGS):
            return TRACE_META_UINT32;
        case (PCAPNG_META_EPB_HASH):
            return TRACE_META_STRING;
        case (PCAPNG_META_EPB_DROPCOUNT):
            return TRACE_META_UINT64;
        }
        break;
    case (PCAPNG_DECRYPTION_SECRETS_TYPE):
...
static void *pcapng_jump_to_options(libtrace_packet_t *packet)
{
    ...
    if (blocktype == PCAPNG_SECTION_TYPE) {
        ptr += sizeof(pcapng_sec_t);
    } else if (blocktype == PCAPNG_INTERFACE_TYPE) {
        ptr += sizeof(pcapng_int_t);
    } else if (blocktype == PCAPNG_OLD_PACKET_TYPE) {
        ptr += sizeof(pcapng_opkt_t);
    } else if (blocktype == PCAPNG_NAME_RESOLUTION_TYPE) {
        ptr += sizeof(pcapng_nrb_t);
    } else if (blocktype == PCAPNG_INTERFACE_STATS_TYPE) {
        ptr += sizeof(pcapng_stats_t);
...
libtrace_meta_t *pcapng_get_all_meta(libtrace_packet_t *packet)
{
    ...
    while (optcode != PCAPNG_OPTION_END &&
        ...
        if (result->items[result->num - 1].datatype == TRACE_META_STRING) {
            result->items[result->num - 1].data = calloc(1, len + 1);
            ...
            if (result->items[result->num - 1].datatype == TRACE_META_UINT32) {
                uint32_t t =
                    *(uint32_t *)(ptr + sizeof(struct pcapng_optheader));
                t = ntohl(t);
                memcpy(result->items[result->num - 1].data, &t,
                       sizeof(uint32_t));
            } else if (result->items[result->num - 1].datatype ==
                       TRACE_META_UINT64) {
                uint64_t t =
                    *(uint64_t *)(ptr + sizeof(struct pcapng_optheader));
...
void pcapng_constructor(void) { register_format(&pcapng); }
