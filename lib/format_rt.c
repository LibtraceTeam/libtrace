...
static int rt_connect(libtrace_t *libtrace)
{
    ...
    if (recv(RT_INFO->input_fd, (void *)&connect_msg, sizeof(rt_header_t), 0) !=
        sizeof(rt_header_t)) {
        trace_set_err(libtrace, TRACE_ERR_INIT_FAILED,
    ...
    case TRACE_RT_DENY_CONN:
        ...
        if (recv(RT_INFO->input_fd, (void *)&deny_hdr, sizeof(rt_deny_conn_t),
    ...
    case TRACE_RT_HELLO:
         ...
         * server */
        if (recv(RT_INFO->input_fd, (void *)&hello_opts, sizeof(rt_hello_t),
                 0) != sizeof(rt_hello_t)) {
            trace_set_err(libtrace, TRACE_ERR_INIT_FAILED,
    ...
    default:
        trace_set_err(libtrace, TRACE_ERR_INIT_FAILED,
                      "Unknown message type received: %d", connect_msg.type);
        return -1;
    }
    trace_set_err(libtrace, TRACE_ERR_INIT_FAILED,
...
static int rt_init_input(libtrace_t *libtrace)
{
     ...
     * port */
    if (strlen(uridata) == 0) {
        RT_INFO->hostname = strdup("localhost");
        RT_INFO->port = COLLECTOR_PORT;
    } else {
        /* If the user does not specify a port, assume the default
         * port */
        if ((scan = strchr(uridata, ':')) == NULL) {
            RT_INFO->hostname = strdup(uridata);
            RT_INFO->port = COLLECTOR_PORT;
        } else {
            RT_INFO->hostname =
                (char *)strndup(uridata, (size_t)(scan - uridata));
            RT_INFO->port = atoi(++scan);
        }
    }
...
static int rt_start_input(libtrace_t *libtrace)
{
    ...
    if (send(RT_INFO->input_fd, (void *)&start_msg,
             sizeof(rt_header_t) + start_msg.length,
             0) != sizeof(rt_header_t)) {
...
static int rt_pause_input(libtrace_t *libtrace)
{
    ...
    if (send(RT_INFO->input_fd, (void *)&close_msg,
             sizeof(rt_header_t) + close_msg.length,
             0) != (int)sizeof(rt_header_t) + close_msg.length) {
...
static int rt_send_ack(libtrace_t *libtrace, uint32_t seqno)
{
    ...
    if (!ack_buffer) {
        ack_buffer = (char *)malloc(sizeof(rt_header_t) + sizeof(rt_ack_t));
    }
...
static int rt_set_format(libtrace_t *libtrace, libtrace_packet_t *packet)
{
    ...
    case TRACE_RT_DATA_LEGACY_POS:
        ...
        trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                      "Legacy packet cannot be sent over rt");
        return -1;
    default:
...
#    define MSG_NOSIGNAL 0
    ...
    if (RT_INFO->buf_write - RT_INFO->pkt_buffer > RT_BUF_SIZE / 2) {
        ...
        memcpy(newbuf, RT_INFO->buf_read,
               RT_INFO->buf_write - RT_INFO->buf_read);
    ...
    if ((numbytes =
             recv(RT_INFO->input_fd, RT_INFO->buf_write,
        ...
        trace_set_err(libtrace, TRACE_ERR_RT_FAILURE,
                      "Error reading from RT socket: %s", strerror(errno));
        return -1;
    }
                              ...
                              int block)
{
    ...
    if (!packet->internalid) {
        trace_set_err(libtrace, TRACE_ERR_RT_FAILURE,
                      "packet->internalid is 0 in rt_get_next_packet()");
        return -1;
    }
...
rt_get_link_type(UNUSED const libtrace_packet_t *packet)
{
                                          ...
                                          libtrace_packet_t *packet)
{
    ...
    if (!packet) {
        trace_set_err(trace, TRACE_ERR_NULL_PACKET,
                      "NULL packet passed into trace_event_rt()");
        /* Return empty event on error? */
        return event;
    }
...
void rt_constructor(void) { register_format(&rt); }
