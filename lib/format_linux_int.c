                                      ...
                                      uint32_t flags)
{
    ...
    packet->payload =
        (char *)buffer + sizeof(struct libtrace_linuxnative_header);
    packet->type = rt_type;
                                          ...
                                          libtrace_message_queue_t *queue)
{
    ...
    snaplen = LIBTRACE_MIN((int)LIBTRACE_PACKET_BUFSIZE - (int)sizeof(*hdr),
    ...
    // Check for a packet - TODO only Linux has MSG_DONTWAIT should use fctl
    // O_NONBLOCK
    ...
    if ((int)hdr->wirelen == -1 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
        ...
        do {
            /* Use select to allow us to time out occasionally to check if
             * someone has hit Ctrl-C or otherwise wants us to stop reading and
             * return so they can exit their program.
             */
            tout.tv_sec = 0;
            tout.tv_usec = 500000;
    ...
    hdr->caplen =
        LIBTRACE_MIN((unsigned int)snaplen, (unsigned int)hdr->wirelen);
...
static int linuxnative_read_packet(libtrace_t *libtrace,
                                   libtrace_packet_t *packet)
{
...
#    ifdef HAVE_PACKET_FANOUT
static int linuxnative_pread_packets(libtrace_t *libtrace, libtrace_thread_t *t,
                                    ...
                                    libtrace_packet_t *packet)
{
    ...
    ret = sendto(FORMAT_DATA_OUT->fd, packet->payload,
                 trace_get_capture_length(packet), 0, (struct sockaddr *)&hdr,
                 (socklen_t)sizeof(hdr));
...
linuxnative_get_link_type(const struct libtrace_packet_t *packet)
{
    uint16_t linktype =
...
linuxnative_get_direction(const struct libtrace_packet_t *packet)
{
...
static libtrace_direction_t
linuxnative_set_direction(libtrace_packet_t *packet,
                          libtrace_direction_t direction)
{
    return linuxcommon_set_direction(
        &((struct libtrace_linuxnative_header *)(packet->buffer))->hdr,
        direction);
}
...
static int linuxnative_get_wire_length(const libtrace_packet_t *packet)
{
    ...
    int wirelen =
        ((struct libtrace_linuxnative_header *)(packet->buffer))->wirelen;
...
static int
linuxnative_get_framing_length(UNUSED const libtrace_packet_t *packet)
{
                                             ...
                                             size_t size)
{
    ...
    if (!packet) {
        fprintf(stderr,
                "NULL packet passed into linuxnative_set_capture_length()\n");
        /* Return -1 on error? */
        return ~0U;
    }
...
void linuxnative_constructor(void) { register_format(&linuxnative); }
