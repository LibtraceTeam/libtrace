...
static int tsh_read_packet(libtrace_t *libtrace, libtrace_packet_t *packet)
{
    ...
    if ((numbytes = wandio_read(libtrace->io, buffer2,
                                (size_t)sizeof(libtrace_ip_t) +
                                    16)) /* 16 bytes of transport header */
...
tsh_get_link_type(const libtrace_packet_t *packet UNUSED)
{
...
{
    return ntohl(((tsh_pkt_header_t *)(packet->header))->usecs &
                 htonl(0xFF000000)) >>
           24;
}
