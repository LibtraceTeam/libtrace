...
static int pcapfile_read_packet(struct libtrace_t *libtrace, struct libtrace_packet_t *packet) {
	...
	err=libtrace_io_read(DATA(libtrace)->file,
			packet->buffer,
			sizeof(struct pcapfile_pkt_hdr_t));
	...
	err=libtrace_io_read(DATA(libtrace)->file,
			(char*)packet->buffer+sizeof(struct pcapfile_pkt_hdr_t),
			swapl(libtrace,((struct pcapfile_pkt_hdr_t*)packet->buffer)->caplen)
			);
	...
	return sizeof(struct pcapfile_pkt_hdr_t)
		+swapl(libtrace,((struct pcapfile_pkt_hdr_t*)packet->buffer)->caplen);
}
...
static size_t pcapfile_set_capture_length(libtrace_packet_t *packet,size_t size) {
	struct pcapfile_pkt_hdr_t *pcapptr = 0;
	assert(packet);
	if (size > trace_get_capture_length(packet)) {
		/* can't make a packet larger */
		return trace_get_capture_length(packet);
	}
	pcapptr = (struct pcapfile_pkt_hdr_t *)packet->header;
