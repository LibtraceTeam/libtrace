...
static bool pcapfile_can_write(libtrace_packet_t *packet)
{
    ...
    if (ltype == TRACE_TYPE_PCAPNG_META ||
        ltype == TRACE_TYPE_CONTENT_INVALID || ltype == TRACE_TYPE_UNKNOWN ||
...
{
    return (header->magic_number == MAGIC1_REV ||
            header->magic_number == MAGIC2_REV);
}
...
{
    libtrace->format_data = malloc(sizeof(struct pcapfile_format_data_out_t));
    if (!libtrace->format_data) {
    ...
    if (!DATA(libtrace)->started) {
        ...
        if (!libtrace->io) {
            trace_set_err(libtrace, TRACE_ERR_BAD_IO,
                          "Trace cannot start IO in pcapfile_start_input()");
            return -1;
        }

        err = wandio_read(libtrace->io, &DATA(libtrace)->header,
                          sizeof(DATA(libtrace)->header));

        DATA(libtrace)->started = true;
        if (!(sizeof(DATA(libtrace)->header) > 0)) {
            trace_set_err(libtrace, TRACE_ERR_INIT_FAILED,
                          "Trace is missing header in pcapfile_start_input()");
            return -1;
        }
        ...
        if (swaps(libtrace, DATA(libtrace)->header.version_major) != 2 &&
            ...
            trace_set_err(
                ...
                swaps(libtrace, DATA(libtrace)->header.version_major),
                swaps(libtrace, DATA(libtrace)->header.version_minor));
            return -1;
        }
    }
...
static int pcapfile_config_input(libtrace_t *libtrace, trace_option_t option,
                                 void *data)
{
    switch (option) {
...
static int pcapfile_read_packet(libtrace_t *libtrace, libtrace_packet_t *packet)
{
    ...
    packet->type =
        pcap_linktype_to_rt(swapl(libtrace, DATA(libtrace)->header.network));
    ...
    err = wandio_read(libtrace->io, packet->buffer,
    ...
    if (err < (int)sizeof(libtrace_pcapfile_pkt_hdr_t)) {
        trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                      "Incomplete pcap packet header");
        return -1;
    }

    bytes_to_read = swapl(
        libtrace, ((libtrace_pcapfile_pkt_hdr_t *)packet->buffer)->caplen);

    if (bytes_to_read >=
        (LIBTRACE_PACKET_BUFSIZE - sizeof(libtrace_pcapfile_pkt_hdr_t))) {
        trace_set_err(
            libtrace, TRACE_ERR_BAD_PACKET,
            "Invalid caplen in pcap header (%u) - trace may be corrupt",
    ...
    err = wandio_read(
        libtrace->io,
        (char *)packet->buffer + sizeof(libtrace_pcapfile_pkt_hdr_t),
        (size_t)swapl(libtrace,
                      ((libtrace_pcapfile_pkt_hdr_t *)packet->buffer)->caplen));
    ...
    if (err < (int)bytes_to_read) {
        trace_set_err(libtrace, TRACE_ERR_WANDIO_FAILED,
                      "Incomplete pcap packet body");
        return -1;
    }

    if (pcapfile_prepare_packet(libtrace, packet, packet->buffer, packet->type,
                                flags)) {
        return -1;
    }
...
static int pcapfile_write_packet(libtrace_out_t *out, libtrace_packet_t *packet)
{
    ...
    while (libtrace_to_pcap_linktype(linktype) == TRACE_DLT_ERROR) {
        if (!demote_packet(packet)) {
            trace_set_err_out(out, TRACE_ERR_NO_CONVERSION,
    ...
    if (!DATAOUT(out)->file) {
        ...
        DATAOUT(out)->file =
            trace_open_file_out(out, DATAOUT(out)->compress_type,
                                DATAOUT(out)->level, DATAOUT(out)->flag);
        ...
        pcaphdr.sigfigs = 0;
        pcaphdr.snaplen = 65536;
        pcaphdr.network = libtrace_to_pcap_linktype(linktype);
    ...
    if (hdr.caplen >= LIBTRACE_PACKET_BUFSIZE) {
        trace_set_err_out(out, TRACE_ERR_BAD_PACKET,
                          "Capture length is greater than buffer size in "
                          "pcap_write_packet()");
        return -1;
    }
    ...
    numbytes = wandio_wwrite(DATAOUT(out)->file, &hdr, sizeof(hdr));

    if (numbytes != sizeof(hdr)) {
        trace_set_err_out(out, TRACE_ERR_WANDIO_FAILED,
                          "Failed to write to pcapfile: %s", strerror(errno));
        return -1;
    }

    /* Write the rest of the packet now */
    ret = wandio_wwrite(DATAOUT(out)->file, ptr, hdr.caplen);
...
pcapfile_get_link_type(const libtrace_packet_t *packet)
{
    return pcap_linktype_to_libtrace(rt_to_pcap_linktype(packet->type));
}

static libtrace_direction_t
pcapfile_get_direction(const libtrace_packet_t *packet)
{
    /* This function can be found in format_helper.c */
    return pcap_get_direction(packet);
}

static struct timeval pcapfile_get_timeval(const libtrace_packet_t *packet)
{
    ...
    if (!packet->header) {
        trace_set_err(packet->trace, TRACE_ERR_BAD_HEADER,
                      "pcap packet with NULL header passed to "
...
static struct timespec pcapfile_get_timespec(const libtrace_packet_t *packet)
{
    libtrace_pcapfile_pkt_hdr_t *hdr;
    struct timespec ts;
...
static int pcapfile_get_capture_length(const libtrace_packet_t *packet)
{
    ...
    if (!packet) {
        fprintf(stderr,
                "NULL packet passed to pcapfile_get_capture_length()\n");
...
static size_t pcapfile_set_capture_length(libtrace_packet_t *packet,
                                          size_t size)
{
    ...
    if (!packet) {
        fprintf(stderr,
                "NULL packet passed into pcapfile_set_capture_length\n");
        /* Return -1 on error? */
        return ~0U;
    }
...
static struct libtrace_eventobj_t pcapfile_event(libtrace_t *libtrace,
...
void pcapfile_constructor(void) { register_format(&pcapfile); }
