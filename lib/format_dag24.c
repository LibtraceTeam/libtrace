...
static int dag_available(libtrace_t *libtrace)
{
    ...
    FORMAT_DATA->top =
        dag_offset(FORMAT_DATA->fd, &(FORMAT_DATA->bottom), DAGF_NONBLOCK);
...
static int dag_init_input(libtrace_t *libtrace)
{
    ...
    if (S_ISCHR(buf.st_mode)) {
        ...
        if ((FORMAT_DATA->fd = dag_open(dag_dev_name)) < 0) {
            trace_set_err(libtrace, errno, "Cannot open DAG %s", dag_dev_name);
            free(dag_dev_name);
            return -1;
        }

        /* Memory-map ourselves a pointer to the DAG memory hole */
        if ((FORMAT_DATA->buf = (void *)dag_mmap(FORMAT_DATA->fd)) ==
            MAP_FAILED) {
            trace_set_err(libtrace, errno, "Cannot mmap DAG %s", dag_dev_name);
            free(dag_dev_name);
            return -1;
        }
    } else {
...
static int dag_get_duckinfo(libtrace_t *libtrace, libtrace_packet_t *packet)
{
    ...
    if ((ioctl(FORMAT_DATA->fd, DAG_IOINF, &lt_dag_inf) < 0)) {
        trace_set_err(libtrace, errno, "Error using DAG_IOINF");
        return -1;
    }
    if (!IsDUCK(&lt_dag_inf)) {
        printf("WARNING: %s does not have modern clock support - No DUCK "
...
static dag_record_t *dag_get_record(libtrace_t *libtrace)
{
    ...
    if (size < dag_record_size) {
        fprintf(stderr,
                "DAG2.4 rlen is invalid (rlen = %u, must be at least %u)\n",
...
static int dag_read_packet(libtrace_t *libtrace, libtrace_packet_t *packet)
{
    ...
    if (dag_prepare_packet(libtrace, packet, erfptr, TRACE_RT_DATA_ERF, flags))
        return -1;
