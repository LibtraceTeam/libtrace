...
DLLEXPORT void libtrace_vector_push_back(libtrace_vector_t *v, void *d)
{
        ...
        if (v->size >= v->max_size) {
                ...
                v->max_size *= 2;
                v->elements =
                    realloc(v->elements, v->max_size * v->element_size);
                if (!v->elements) {
                        fprintf(stderr,
                                "Unable to allocate memory for v->elements in "
                                "libtrace_vector_push_back()\n");
...
DLLEXPORT int libtrace_vector_get(libtrace_vector_t *v, size_t location,
                                  void *d)
{
        ASSERT_RET(pthread_mutex_lock(&v->lock), == 0);
        if (location >= v->size) {
...
DLLEXPORT void libtrace_vector_append(libtrace_vector_t *dest,
                                      libtrace_vector_t *src)
{
        if (dest->element_size != src->element_size) {
                fprintf(stderr, "Elements must be the same size in "
                                "libtrace_vector_append()\n");
                return;
        }
        ...
        if (src->size == 0) // Double check now we've got the locks - Nothing to
                            // do if this is the case
                goto unlock;
        if (dest->size == 0) {
                memswap(&dest->max_size, &src->max_size, sizeof(src->max_size));
                memswap(&dest->size, &src->size, sizeof(src->size));
                memswap(&dest->element_size, &src->element_size,
                        sizeof(src->element_size));
                memswap(&dest->elements, &src->elements, sizeof(src->elements));
        } else {
                size_t oldmax = dest->max_size;
                while (dest->max_size - dest->size < src->size)
                        dest->max_size *= 2;
                if (oldmax != dest->max_size)
                        dest->elements =
                            realloc(dest->elements,
                                    dest->max_size * dest->element_size);
                // Now do the move
                memcpy(&dest->elements[dest->element_size * dest->size],
                       src->elements, src->element_size * src->size);
...
DLLEXPORT void libtrace_vector_apply_function(libtrace_vector_t *v,
                                              vector_data_fn fn)
{
        size_t cur;
        ASSERT_RET(pthread_mutex_lock(&v->lock), == 0);
...
DLLEXPORT void libtrace_vector_qsort(libtrace_vector_t *v,
