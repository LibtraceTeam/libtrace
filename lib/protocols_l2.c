...
static void *trace_get_payload_from_ppp(void *link, uint16_t *type,
                                        uint32_t *remaining);
...
libtrace_layer2_headers_t *trace_get_layer2_headers(libtrace_packet_t *packet)
{
    ...
    if (packet->trace == NULL) {
        fprintf(stderr,
                "Packet contains a NULL trace in trace_get_layer2_headers()\n");
        return NULL;
    }
    ...
    if (r == NULL) {
        trace_set_err(
            packet->trace, TRACE_ERR_OUT_OF_MEMORY,
    ...
    while (remaining != 0 && ptr != NULL) {
        ...
        if ((r->num + 1) >= allocated_headers) {
            allocated_headers += 10;
            r->header = realloc(r->header, sizeof(libtrace_layer2_header_t) *
                                               allocated_headers);
        ...
        case (TRACE_ETHERTYPE_8021Q):
            ...
            ptr = (char *)trace_get_payload_from_vlan(ptr, &ethertype,
                                                      &remaining);
            break;
        ...
        case (TRACE_ETHERTYPE_MPLS):
            ...
            ptr = (char *)trace_get_payload_from_mpls(ptr, &ethertype,
                                                      &remaining);
            break;
        ...
        case (TRACE_ETHERTYPE_PPP_DISC):
            ...
            ptr =
                (char *)trace_get_payload_from_ppp(ptr, &ethertype, &remaining);
            break;
                                  ...
                                  uint32_t *remaining)
{
    ...
    while (ethertype != TRACE_ETHERTYPE_8021Q &&
...
libtrace_packet_t *trace_strip_packet(libtrace_packet_t *packet)
{
    ...
    ethernet =
        (libtrace_ether_t *)trace_get_layer2(packet, &linktype, &remaining);
    ...
    while (!done) {
        ...
        case TRACE_ETHERTYPE_8021Q:
            nextpayload = (char *)trace_get_payload_from_vlan(
                nextpayload, &ethertype, &remaining);
                                  ...
                                  uint32_t *remaining)
{
    ...
    if (!type) {
        fprintf(stderr,
                "NULL type passed into trace_get_payload_from_mpls()\n");
        return NULL;
    }
...
static void *trace_get_payload_from_llcsnap(void *link, uint16_t *type,
                                            uint32_t *remaining)
{
    /* 64 byte capture. */
...
static void *trace_get_payload_from_80211(void *link, uint16_t *type,
                                          uint32_t *remaining)
{
    ...
    if (remaining)
        *remaining =
            *remaining - sizeof(libtrace_80211_t) - extra - sizeof(*eth);
                                   ...
                                   uint32_t *remaining)
{
    ...
    if (!type) {
        fprintf(stderr,
                "NULL type passed into trace_get_payload_from_pppoe()\n");
        return NULL;
    }
    ...
    return trace_get_payload_from_ppp(link + sizeof(libtrace_pppoe_t), type,
                                      remaining);
}
...
static void *trace_get_payload_from_ppp_hdlc(void *link, uint16_t *type,
                                             uint32_t *remaining)
{
                                 ...
                                 uint32_t *remaining)
{
     ...
     */
    while (!done) {
        void *nexthdr = trace_get_payload_from_meta(meta, linktype, remaining);
...
DLLEXPORT
void *trace_get_payload_from_atm(void *link, uint8_t *type, uint32_t *remaining)
{
                                              ...
                                              uint32_t *remaining)
{
    ...
    case TRACE_TYPE_ATM:
         ...
         */
        return (l ? trace_get_payload_from_llcsnap(l, ethertype, remaining)
                  : NULL);
    case TRACE_TYPE_LLCSNAP:
        return trace_get_payload_from_llcsnap(link, ethertype, remaining);

    case TRACE_TYPE_HDLC_POS:
        return trace_get_payload_from_chdlc(link, ethertype, remaining);
    case TRACE_TYPE_POS:
        return trace_get_payload_from_ppp_hdlc(link, ethertype, remaining);
    /* TODO: Unsupported */
    case TRACE_TYPE_AAL5:
        return NULL;
 ...
 */
static uint8_t *get_source_mac_from_wifi(void *wifi)
{
...
DLLEXPORT uint8_t *trace_get_source_mac(libtrace_packet_t *packet)
{
    ...
    case TRACE_TYPE_ETSILI:
    case TRACE_TYPE_CORSAROTAG:
        fprintf(stderr, "Metadata headers should already be skipped in "
...
DLLEXPORT uint8_t *trace_get_destination_mac(libtrace_packet_t *packet)
{
    ...
    if (!packet) {
        fprintf(stderr,
                "NULL packet passed into trace_get_destination_mac()\n");
        return NULL;
    }
