...
static int parse_pciaddr(char *str, struct rte_pci_addr *addr, long *core)
{
    ...
    matches =
        sscanf(str, "%8" SCNx32 ":%2" SCNx8 ":%2" SCNx8 ".%2" SCNx8 "-%ld",
               &addr->domain, &addr->bus, &addr->devid, &addr->function, core);
#else
    matches =
        sscanf(str, "%4" SCNx16 ":%2" SCNx8 ":%2" SCNx8 ".%2" SCNx8 "-%ld",
               &addr->domain, &addr->bus, &addr->devid, &addr->function, core);
#endif
...
static int pci_to_numa(struct rte_pci_addr *dev_addr)
{
    ...
    snprintf(path, sizeof(path),
             "/sys/bus/pci/devices/" PCI_PRI_FMT "/numa_node", dev_addr->domain,
...
#    endif
    ...
    if (!(my_cpu > 0 && my_cpu <= MIN(RTE_MAX_LCORE, nb_cpu))) {
        snprintf(err, errlen,
                 ...
                 (uint32_t)my_cpu, (uint32_t)MIN(RTE_MAX_LCORE, nb_cpu));
        return -1;
    }
     ...
     */
    snprintf(use_addr_str, sizeof(use_addr_str), PCI_PRI_FMT, use_addr.domain,
             use_addr.bus, use_addr.devid, use_addr.function);
#endif
    ...
    if ((ret = rte_eal_init(argc, argv)) < 0) {
        snprintf(err, errlen, "Libtrace DPDK: Initialisation of EAL failed: %s",
                 strerror(-ret));
        return -1;
    }
    ...
    if ((ret = rte_pmd_init_all()) < 0) {
        snprintf(err, errlen, "Libtrace DPDK: rte_pmd_init_all failed: %s",
                 strerror(-ret));
        return -1;
    }
#endif
    ...
    if ((ret = rte_eal_pci_probe()) < 0) {
        snprintf(err, errlen, "Intel DPDK - rte_eal_pci_probe failed: %s",
                 strerror(-ret));
        return -1;
    }
#endif
...
static int dpdk_init_input(libtrace_t *libtrace, enum device_type dev_type)
{
    ...
    libtrace->format_data =
        (struct dpdk_format_data_t *)malloc(sizeof(struct dpdk_format_data_t));
...
#    endif
...
#endif
        format_data->pktmbuf_pool = dpdk_alloc_memory(
            format_data->nb_tx_buf * 2, buf_size, format_data->nic_numa_node);
    ...
    uint8_t rss_size = 52; // 52 for i40e, 40 for others, use the largest by
                           // default In new versions DPDK we can query the size
     ...
     */
    ret = rte_eth_tx_queue_setup(format_data->port, 0 /* queue XXX */,
                                 format_data->nb_tx_buf, SOCKET_ID_ANY,
    ...
    for (i = 0; i < rx_queues; i++) {
        ...
        if (stream->lcore == -1) {
            ...
            if (coremap != NULL)
                stream->lcore = dpdk_reserve_lcore(
                    true, format_data->nic_numa_node, coremap[i]);
            else
                stream->lcore =
                    dpdk_reserve_lcore(true, format_data->nic_numa_node, -1);
        }
        ...
        if (stream->mempool == NULL) {
            stream->mempool =
                dpdk_alloc_memory(format_data->nb_rx_buf * 2, buf_size,
            ...
            if (stream->mempool == NULL) {
                ...
                stream->mempool = dpdk_alloc_memory(format_data->nb_rx_buf * 2,
                                                    buf_size, SOCKET_ID_ANY);
            }
        ...
        ret = rte_eth_rx_queue_setup(
            ...
            format_data->nic_numa_node,
            DPDK_USE_NULL_QUEUE_CONFIG ? NULL : &rx_conf, stream->mempool);
        if (ret < 0) {
            snprintf(err, errlen,
...
#endif
    if (ret != 0) {
        fprintf(stderr, "Libtrace DPDK: rte_eth_stats_reset failed %d : %s\n",
                ret, strerror(-ret));
    }
...
#    endif
    ...
    if (reading) {
        ...
        if (t->type == THREAD_PERPKT) {
            t->format_data = libtrace_list_get_index(
...
static int dpdk_start_output(libtrace_out_t *libtrace)
{
    ...
    if (dpdk_start_streams(FORMAT(libtrace), err, sizeof(err), 1, true, NULL) !=
        0) {
        trace_set_err_out(libtrace, TRACE_ERR_INIT_FAILED, "%s", err);
...
int dpdk_pause_input(libtrace_t *libtrace)
{
    ...
    if (FORMAT(libtrace)->paused == DPDK_RUNNING) {
        ...
        for (; FORMAT(libtrace)->burst_offset < FORMAT(libtrace)->burst_size;
             ++FORMAT(libtrace)->burst_offset) {
            rte_pktmbuf_free(
                FORMAT(libtrace)->burst_pkts[FORMAT(libtrace)->burst_offset]);
        }
        FORMAT(libtrace)->burst_offset = 0;
        FORMAT(libtrace)->burst_size = 0;
...
static int dpdk_write_packet(libtrace_out_t *trace, libtrace_packet_t *packet)
{
    ...
    if (m_buff[0] == NULL) {
        ...
        do {
            ret = rte_eth_tx_burst(FORMAT(trace)->port, 0 /*queue TODO*/,
                                   m_buff, 1);
        } while (ret != 1);
    }
                                   ...
                                   struct rte_mbuf **pkts, size_t nb_pkts)
{
    ...
    for (i = 0; i < nb_pkts; ++i) {
        ...
        hdr->cap_len =
            rte_pktmbuf_pkt_len(pkts[i]); // TODO should this be data_len?
        ...
        estimated_wraps =
            (cur_sys_time_ns - plc->ts_last_sys) / (1ull << TS_NBITS_82580);
         ...
         * considering */
        hdr->timestamp +=
            plc->ts_first_sys + plc->wrap_count * (1ull << TS_NBITS_82580);
                            ...
                            struct rte_mbuf *pkts_burst[], size_t nb_packets)
{
    ...
    while (1) {
        ...
        nb_rx = rte_eth_rx_burst(FORMAT(libtrace)->port, stream->queue_id,
                              ...
                              libtrace_packet_t **packets, size_t nb_packets)
{
    ...
    nb_rx = dpdk_read_packet_stream(libtrace, stream, &t->messages, pkts_burst,
                                    nb_packets);
    ...
    if (nb_rx > 0) {
        for (i = 0; i < nb_rx; ++i) {
            ...
            packets[i]->error = 1;
            hdr =
                (struct dpdk_addt_hdr *)((struct rte_mbuf *)pkts_burst[i] + 1);
                                     ...
                                     libtrace_packet_t *packet)
{
    ...
    do {
        ...
        if (FORMAT(trace)->burst_size != FORMAT(trace)->burst_offset) {
            ...
            packet->buffer =
                FORMAT(trace)->burst_pkts[FORMAT(trace)->burst_offset++];
