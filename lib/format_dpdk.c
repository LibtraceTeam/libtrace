...
#define MBUF_PKTDATA(x)                                                        \
        ((char *)x + sizeof(struct rte_mbuf) + RTE_PKTMBUF_HEADROOM)
...
#        warning                                                               \
            "RTE_PKT_MBUF_HEADROOM is not set to the default value of 128 - "  \
...
struct dpdk_format_data_t {
        ...
        portid_t port; /* Always 0 we only whitelist a single port - Shared TX &
                          RX */
        ...
        struct rte_pci_addr
            blacklist[BLACK_LIST_SIZE]; /* Holds our device blacklist */
        unsigned int nb_blacklist; /* Number of blacklist items in are valid */
#endif
        char mempool_name[MEMPOOL_NAME_LEN]; /* The name of the mempool that we
                                                are using */
 ...
 */
#if DPDK_USE_BLACKLIST
static int blacklist_devices(struct dpdk_format_data_t *format_data,
                             struct rte_pci_addr *whitelist)
{
        ...
        {
                if (whitelist != NULL &&
                    whitelist->domain == dev->addr.domain &&
                    whitelist->bus == dev->addr.bus &&
                    whitelist->devid == dev->addr.devid &&
                    whitelist->function == dev->addr.function)
                        continue;
                if (format_data->nb_blacklist >=
                    sizeof(format_data->blacklist) /
                        sizeof(format_data->blacklist[0])) {
                        fprintf(
                            stderr,
                            "Warning: too many devices to blacklist consider"
        ...
        rte_eal_pci_set_blacklist(format_data->blacklist,
                                  format_data->nb_blacklist);
        return 0;
}
...
static int parse_pciaddr(char *str, struct rte_pci_addr *addr, long *core)
{
        ...
        matches = sscanf(
            str, "%8" SCNx32 ":%2" SCNx8 ":%2" SCNx8 ".%2" SCNx8 "-%ld",
            &addr->domain, &addr->bus, &addr->devid, &addr->function, core);
#else
        matches = sscanf(
            str, "%4" SCNx16 ":%2" SCNx8 ":%2" SCNx8 ".%2" SCNx8 "-%ld",
            &addr->domain, &addr->bus, &addr->devid, &addr->function, core);
#endif
...
static int pci_to_numa(struct rte_pci_addr *dev_addr)
{
        ...
        snprintf(path, sizeof(path),
                 "/sys/bus/pci/devices/" PCI_PRI_FMT "/numa_node",
        ...
        if ((file = fopen(path, "r")) != NULL) {
                ...
                if (fscanf(file, "%d", &numa_node) != 1) {
                        ...
                        fprintf(stderr,
                                "Warning /sys/bus/pci/devices/" PCI_PRI_FMT
                                "/numa_node = %d appears to be invalid\n",
                                dev_addr->domain, dev_addr->bus,
                                dev_addr->devid, dev_addr->function, numa_node);
                }
                fclose(file);
                return numa_node;
        }
...
static inline void dump_configuration()
{
        ...
        fprintf(stderr,
                ...
                "---Main LCore : %" PRIu32 "\n"
                "---LCore Count  : %" PRIu32 "\n",
                rte_version(), rte_get_main_lcore(), rte_lcore_count());
...
static inline int dpdk_init_environment(char *uridata,
                                        struct dpdk_format_data_t *format_data,
                                        char *err, int errlen)
{
        int ret; /* Returned error codes */
        struct rte_pci_addr use_addr = {
            0}; /* The only address that we don't blacklist */
        ...
        long
            my_cpu; /* The CPU core we want to bind to, counting from 1 not 0 */
         ...
         *      - Most likely to be half the number of ram slots in your
         * machine. We could count ram slots by "dmidecode -t 17 | grep -c
         * 'Size:'" Controls where in memory packets are stored such that they
         * are spread across the channels. We just use 1 to be safe.
        ...
        char *argv[] = {
                ...
                "1",
                "-l",
                main_lcore_id, /* Give DPDK only the main lcore */
                "--proc-type",
                "auto",
#ifdef __linux__
                "--file-prefix",
...
#        endif
        ...
        if (format_data->dev_type == PCI_DEVICE) {
                /* This allows the user to specify the core - we would try to do
                 * this automatically but it's hard to tell that this is
                 * secondary before running rte_eal_init(...). Currently we are
                 * limited to 1 instance per core due to the way memory is
                 * allocated. */
...
#        endif
                if (format_data->nic_numa_node >= 0) {
                        ...
                        if (!mask) {
                                fprintf(stderr, "Unable to allocate cpu mask "
                                                "in dpdk_init_environment()\n");
                                return -1;
                        }
        ...
        if (!(my_cpu > 0 && my_cpu <= MIN(RTE_MAX_LCORE, nb_cpu))) {
                snprintf(err, errlen,
                         "Libtrace DPDK: User defined a bad CPU number %" PRIu32
                         " must be"
                         " between 1 and %" PRIu32,
                         (uint32_t)my_cpu,
        ...
        snprintf(use_addr_str, sizeof(use_addr_str), PCI_PRI_FMT,
                 use_addr.domain, use_addr.bus, use_addr.devid,
                 use_addr.function);
#endif
        ...
        if ((ret = rte_eal_init(argc, argv)) < 0) {
                snprintf(err, errlen,
                         "Libtrace DPDK: Initialisation of EAL failed: %s",
                         strerror(-ret));
                return -1;
        }
        ...
        if ((long)rte_get_main_lcore() != my_cpu - 1) {
                fprintf(stderr,
                        "Warning: main core running on core %d not %ld.\n",
                        rte_get_main_lcore(), my_cpu - 1);
        }

        // Only the main lcore should be running
        if (rte_lcore_count() != 1) {
                fprintf(stderr, "Expected only the main core to be running in "
                                "dpdk_init_environment()\n");
                return -1;
        }
        ...
        if ((ret = rte_pmd_init_all()) < 0) {
                snprintf(err, errlen,
                         "Libtrace DPDK: rte_pmd_init_all failed: %s",
                         strerror(-ret));
                return -1;
        }
#endif
        ...
        if (format_data->dev_type == PCI_DEVICE) {
                if ((ret = blacklist_devices(format_data, &use_addr)) < 0) {
                        snprintf(
                            err, errlen,
                            "Libtrace DPDK: Whitelisting PCI device failed,"
                            " are you sure the address is correct?: %s",
                            strerror(-ret));
                        return -1;
                }
        }
        ...
        if ((ret = rte_eal_pci_probe()) < 0) {
                snprintf(err, errlen,
                         "Intel DPDK - rte_eal_pci_probe failed: %s",
                         strerror(-ret));
                return -1;
        }
#endif
        ...
        if (rte_eth_dev_count_avail() != 1) {
                snprintf(err, errlen,
                         "Intel DPDK - rte_eth_dev_count_avail returned %d but "
                         "it should be 1",
 ...
 */
static int dpdk_attach_device(char *uridata,
                              struct dpdk_format_data_t *format_data, char *err,
                              size_t errlen)
{
        int ret;
        ...
        if (!dpdk_init) {
                if (dpdk_init_environment(uridata, format_data, err, errlen) !=
                    0) {
                        return RTE_MAX_ETHPORTS;
                }
                dpdk_init = 1;
        ...
        /* rte_dev_probe and RTE_ETH_FOREACH_MATCHING_DEV were added
         * in 18.11-rc1 The old dev_attach way was added in 16.11-rc1 and
         * removed in 18.11-rc1
         * */
#if defined(RTE_ETH_FOREACH_MATCHING_DEV) || defined(USE_DEV_ATTACH)
        ...
        if (format_data->dev_type == PCI_DEVICE) {
                ...
                if (parse_pciaddr(uridata, &pci_addr, &_cpu_core) != 0) {
                        snprintf(err, errlen, "Failed to parse DPDK URI (%s)",
                                 uridata);
                        return RTE_MAX_ETHPORTS;
                }
...
#        ifdef RTE_ETH_FOREACH_MATCHING_DEV
        if ((ret = rte_dev_probe(uri)) != 0) {
                snprintf(err, errlen,
                         "Libtrace DPDK: rte_dev_probe(%s) failed: %s", uri,
                         strerror(-ret));
                return RTE_MAX_ETHPORTS;
        }
#        else
        ...
        if ((ret = rte_eal_dev_attach(uri, args)) != 0) {
                snprintf(err, errlen,
                         "Libtrace DPDK: rte_eal_dev_attach(%s, %s) failed: %s",
                         uri, args, strerror(-ret));
                return RTE_MAX_ETHPORTS;
        }
#        endif
        ...
        if (port == RTE_MAX_ETHPORTS) {
                snprintf(
                    err, errlen,
                    "Libtrace DPDK: unable to find port matching the device %s",
                    uri);
        }

#        ifdef USE_DEV_ATTACH
        /* Revert our changes to the URI, note dpdk_get_port_id expects only the
         * device name */
...
static int dpdk_init_input(libtrace_t *libtrace, enum device_type dev_type)
{
        ...
        libtrace->format_data = (struct dpdk_format_data_t *)malloc(
            sizeof(struct dpdk_format_data_t));
        ...
        FORMAT(libtrace)->per_stream = libtrace_list_init_aligned(
            sizeof(struct dpdk_per_stream_t), CACHE_LINE_SIZE);
        libtrace_list_push_back(FORMAT(libtrace)->per_stream, &stream);

        FORMAT(libtrace)->port = dpdk_attach_device(
            libtrace->uridata, FORMAT(libtrace), err, sizeof(err));
 ...
 *
 * I.e the maximum size of a standard ethernet packet is 1518 (Including MAC
 * checksum) So to allow packets upto 1518 this would be set to 1514 and if
 * GET_MAC_CRC_CHECKSUM is set the maximum size of the returned packet would be
 * 1518 otherwise 1514 would be the largest size possibly returned.
 *
 */
int dpdk_config_input(libtrace_t *libtrace, trace_option_t option, void *data)
{
        switch (option) {
        ...
        case TRACE_OPTION_HASHER:
                ...
                case HASHER_BIDIRECTIONAL:
                        FORMAT(libtrace)->hasher_type =
                            *(enum hasher_types *)data;
    ...
    .tx_thresh =
        {
            /*
             * TX_PTHRESH prefetch
             * Set on the NIC, if the number of unprocessed descriptors to
             * queued on the card fall below this try grab at least hthresh more
             * unprocessed descriptors.
             */
            .pthresh = 36,

            /* TX_HTHRESH host
             * Set on the NIC, the batch size to prefetch unprocessed tx
             * descriptors.
             */
            .hthresh = 0,

            /* TX_WTHRESH writeback
             * Set on the NIC, the number of sent descriptors before writing
             * back status to confirm the transmission. This is done more
             * efficiently as a bulk DMA-transfer rather than writing one at a
             * time. Similar to tx_free_thresh however this is applied to the
             * NIC, where as tx_free_thresh is when DPDK will check these. This
             * is extended upon by tx_rs_thresh (10Gbit cards) which doesn't
             * write all descriptors rather only every n'th item, reducing DMA
             * memory bandwidth.
             */
            .wthresh = 4,
        },
                              ...
                              void *cb_arg)
{
        ...
        if (event != RTE_ETH_EVENT_INTR_LSC) {
                fprintf(stderr,
                        "Received unexpected event in dpdk_lsc_callback()\n");
#if RTE_VERSION >= RTE_VERSION_NUM(17, 8, 0, 1)
                return -1;
#else
                return;
        ...
        if (port != format_data->port) {
                fprintf(stderr, "Port does not match port in format data in "
                                "dpdk_lsc_callback()\n");
#if RTE_VERSION >= RTE_VERSION_NUM(17, 8, 0, 1)
                return -1;
#else
 ...
 * @param socket the preferred NUMA socket - only used if a real core is
 * requested
 * @param core - if > 0 reserve supplied core otherwise let dpdk_reserve_lcore
 * find a core
...
static int dpdk_reserve_lcore(bool real, int socket, int core)
{
        ...
        if (core > -1) {
             ...
             * otherwise start from the MAX core (which is also the main) and
             * work backwards in this case physical cores on the system will not
             * exist so we don't bind these to any particular physical core */
            if (real) {
#ifdef HAVE_LIBNUMA
...
static int dpdk_register_lcore(libtrace_t *libtrace, bool real, int lcore)
{
        ...
        if (real) {
                ...
                ret = pthread_setaffinity_np(pthread_self(), sizeof(cpuset),
                                             &cpuset);
                if (ret != 0) {
                        trace_set_err(libtrace, errno,
                                      "Warning "
 ...
 * re-used.
 */
static struct rte_mempool *dpdk_alloc_memory(unsigned n, unsigned pkt_size,
                                             int socket_id)
{
        ...
        if (mem_pools[socket_index][j]) {
                ...
                test++;
                snprintf(name, MEMPOOL_NAME_LEN, "libtrace_pool_%" PRIu32,
                         test);

                ret =
                    rte_mempool_create(name, n, pkt_size, 128,
 ...
 */
static int dpdk_start_streams(struct dpdk_format_data_t *format_data, char *err,
...
#        endif
        ...
        if (format_data->paused == DPDK_NEVER_STARTED) {
                /* Create the mbuf pool, which is the place packets are
                 * allocated from - There is no free function (I cannot see
                 * one). NOTE: RX queue requires nb_packets + 1 otherwise it
                 * fails to allocate however that extra 1 packet is not used. (I
                 * assume <= vs < error some where in DPDK code) TX requires
                 * nb_tx_buffers + 1 in the case the queue is full so that will
                ...
                format_data->pktmbuf_pool =
                    dpdk_alloc_memory(format_data->nb_tx_buf * 2, buf_size,
                                      format_data->nic_numa_node);

                if (format_data->pktmbuf_pool == NULL) {
                        snprintf(err, errlen,
                                 "Libtrace DPDK: initialisation of mbuf "
                                 "pool failed: %s",
                                 strerror(rte_errno));
                        return -1;
                }
        ...
        uint8_t rss_size =
            52; // 52 for i40e, 40 for others, use the largest by default
        ...
        if (rss_size != 0) {
                ...
                fprintf(stderr,
                        "Libtrace DPDK: couldn't configure RSS hashing!\n");
        }
        ...
        ret =
            rte_eth_dev_configure(format_data->port, rx_queues, 1, &port_conf);
        if (ret < 0) {
                snprintf(err, errlen,
                         "Intel DPDK - Cannot configure device port"
                         " %" PRIu8 " : %s",
        ...
        ret = rte_eth_tx_queue_setup(
            format_data->port, 0 /* queue XXX */, format_data->nb_tx_buf,
            SOCKET_ID_ANY, DPDK_USE_NULL_QUEUE_CONFIG ? NULL : &tx_conf);
        if (ret < 0) {
                snprintf(err, errlen,
                         "Intel DPDK - Cannot configure TX queue"
                         " on port %d : %s",
                         (int)format_data->port, strerror(-ret));
                return -1;
        }
        ...
        for (i = 0; i < rx_queues; i++) {
                ...
                if (libtrace_list_get_size(format_data->per_stream) <=
                    (size_t)i)
                ...
                if (stream->lcore == -1) {
                        ...
                        if (coremap != NULL)
                                stream->lcore = dpdk_reserve_lcore(
                                    true, format_data->nic_numa_node,
                                    coremap[i]);
                        else
                                stream->lcore = dpdk_reserve_lcore(
                                    true, format_data->nic_numa_node, -1);
                }

                if (stream->lcore == -1) {
                        snprintf(err, errlen,
                                 "Intel DPDK - Failed to reserve a lcore"
                                 ". Too many threads?");
                        return -1;
                }
                ...
                if (stream->mempool == NULL) {
                        stream->mempool = dpdk_alloc_memory(
                            format_data->nb_rx_buf * 2, buf_size,
                            rte_lcore_to_socket_id(stream->lcore));

                        if (stream->mempool == NULL) {
                                fprintf(stderr, "Intel DPDK - Warning libtrace "
                                                "couldn't allocate rx memory "
                                                "on the desired socket.\n");
                                fprintf(stderr, "Intel DPDK - This might "
                                                "impact performance, try "
                                                "adding more hugepages.\n");
                                stream->mempool = dpdk_alloc_memory(
                                    format_data->nb_rx_buf * 2, buf_size,
                                    SOCKET_ID_ANY);
                        }
                        ...
                        if (stream->mempool == NULL) {
                                snprintf(err, errlen,
                                         "Intel DPDK - Initialisation of mbuf "
                                         "pool failed: %s",
                                         strerror(rte_errno));
                                return -1;
                        }
                ...
                ret = rte_eth_rx_queue_setup(
                    format_data->port, stream->queue_id, format_data->nb_rx_buf,
                    format_data->nic_numa_node,
                    DPDK_USE_NULL_QUEUE_CONFIG ? NULL : &rx_conf,
                    stream->mempool);
                if (ret < 0) {
                        snprintf(err, errlen,
                                 "Intel DPDK - Cannot configure"
        ...
        if (port_conf.intr_conf.lsc) {
                ...
                ret = rte_eth_dev_callback_register(
                    format_data->port, RTE_ETH_EVENT_INTR_LSC,
...
int dpdk_start_input(libtrace_t *libtrace)
{
        ...
        if (dpdk_start_streams(FORMAT(libtrace), err, sizeof(err), 1, false,
                               libtrace->config.coremap) != 0) {
                trace_set_err(libtrace, TRACE_ERR_INIT_FAILED, "%s", err);
...
int dpdk_pstart_input(libtrace_t *libtrace)
{
        ...
        if (rte_lcore_id() != rte_get_main_lcore() &&
            rte_lcore_id() != LCORE_ID_ANY)
                fprintf(stderr,
                        "Libtrace DPDK: warning dpdk_pstart_input "
        ...
        if ((ret = dpdk_start_streams(FORMAT(libtrace), err, sizeof(err), tot,
                                      false, libtrace->config.coremap)) == -1) {
                trace_set_err(libtrace, TRACE_ERR_INIT_FAILED, "%s", err);
                dpdk_fin_input(libtrace);

                return -1;

                // dpdk_start_streams can return -2 which prevents putting
                // libtrace into an error state. We can cleanup stream state and
                // let Libtrace re-attempt starting the stream in non-parallel
                // mode.
        } else if (ret == -2) {

                if (FORMAT(libtrace)->port != RTE_MAX_ETHPORTS)
                        rte_eth_dev_callback_unregister(
                            FORMAT(libtrace)->port, RTE_ETH_EVENT_INTR_LSC,
                            dpdk_lsc_callback, FORMAT(libtrace));
...
#        endif
        ...
        if (reading) {
                ...
                if (t->type == THREAD_PERPKT) {
                        t->format_data =
                            libtrace_list_get_index(
                                FORMAT(libtrace)->per_stream, t->perpkt_num)
                                ->data;
 ...
 * they are destroyed.
 */
void dpdk_punregister_thread(libtrace_t *libtrace UNUSED,
                             libtrace_thread_t *t UNUSED)
{
        ...
        if (rte_lcore_id() >= RTE_MAX_LCORE) {
                fprintf(
                    stderr,
                    "Expected core id less than or equal to RTE_MAX_LCORE in "
        ...
        if (!core_config[rte_lcore_id()]) {
                fprintf(
                    stderr,
                    "INFO: we are skipping unregistering a unknown lcore\n");
                pthread_mutex_unlock(&dpdk_lock);
                return;
        }
...
static int dpdk_start_output(libtrace_out_t *libtrace)
{
        ...
        if (dpdk_start_streams(FORMAT(libtrace), err, sizeof(err), 1, true,
                               NULL) != 0) {
                trace_set_err_out(libtrace, TRACE_ERR_INIT_FAILED, "%s", err);
...
int dpdk_pause_input(libtrace_t *libtrace)
{
        ...
        if (FORMAT(libtrace)->paused == DPDK_RUNNING) {
                ...
                for (; FORMAT(libtrace)->burst_offset <
                       FORMAT(libtrace)->burst_size;
                     ++FORMAT(libtrace)->burst_offset) {
                        rte_pktmbuf_free(
                            FORMAT(libtrace)
                                ->burst_pkts[FORMAT(libtrace)->burst_offset]);
                }
                FORMAT(libtrace)->burst_offset = 0;
                FORMAT(libtrace)->burst_size = 0;
...
static int dpdk_write_packet(libtrace_out_t *trace, libtrace_packet_t *packet)
{
        ...
        if (trace_get_link_type(packet) == TRACE_TYPE_ETH && wirelen == caplen)
                caplen -= RTE_ETHER_CRC_LEN;

        m_buff[0] = rte_pktmbuf_alloc(FORMAT(trace)->pktmbuf_pool);
        if (m_buff[0] == NULL) {
                trace_set_err_out(trace, TRACE_ERR_OUT_OF_MEMORY,
                                  "Cannot get an empty packet buffer");
                return -1;
        } else {
                int ret;
                ...
                if (mbuf_dst != NULL) {
                        ...
                        trace_set_err_out(trace, TRACE_ERR_NO_CONVERSION,
                                          "Packet too large");
                        return -1;
                }
                do {
                        ret = rte_eth_tx_burst(FORMAT(trace)->port,
                                               0 /*queue TODO*/, m_buff, 1);
                } while (ret != 1);
        }
...
get_addt_hdr(const libtrace_packet_t *packet)
{
        ...
        if (!packet->buffer) {
                fprintf(stderr,
                        "NULL packet buffer passed into dpdk_addt_hdr()\n");
                return NULL;
        }
...
int dpdk_prepare_packet(libtrace_t *libtrace UNUSED, libtrace_packet_t *packet,
                        void *buffer, libtrace_rt_types_t rt_type,
                        uint32_t flags)
{
        if (!packet) {
                fprintf(stderr,
                        "NULL packet passed into dpdk_prepare_packet()\n");
...
calculate_wire_time(struct dpdk_format_data_t *format_data, uint32_t pkt_size)
{
        uint32_t wire_time;
         ...
         * The compiler will optimise this into magical multiplication and
         * shifting See
...
retry_calc_wiretime:
        ...
        case 0: {
                ...
                /* We don't know the link speed, make sure numbers are counting
                 * up */
                wire_time = 1;
                break;
        }
...
/**
 * Does any extra preparation to all captured packets
...
static inline void dpdk_ready_pkts(libtrace_t *libtrace,
                                   ...
                                   struct rte_mbuf **pkts, size_t nb_pkts)
{
        ...
        for (i = 0; i < nb_pkts; ++i) {
                ...
                hdr->cap_len = rte_pktmbuf_pkt_len(
                    pkts[i]); // TODO should this be data_len?

#if HAS_HW_TIMESTAMPS_82580
                /* The timestamp is sitting before our packet and is included in
                 * pkt_len */
                ...
                /* Taken from igb_ptp.c part of Intel Linux drivers (Good
                 * example code)
                 *
                 *        +----------+---+   +--------------+
                 *  82580 |    24    | 8 |   |      32      |
                 ...
                 *
                 * NOTE picture is in Big Endian order, in memory it's actually
                 * in Little Endian (for the full 64 bits) i.e. picture is
                 * mirrored
                 */
                ...
                /* This will have serious problems if packets aren't read
                 * quickly that is within a couple of seconds because our clock
                 * cycles every 18 seconds */
                estimated_wraps = (cur_sys_time_ns - plc->ts_last_sys) /
                                  (1ull << TS_NBITS_82580);

                /* Estimated_wraps gives the number of times the counter should
                 * have wrapped (however depending on value last time it could
                 * have wrapped twice more (if hw clock is close to its max
                 * value) or once less (allowing for a bit of variance between
                 * hw and sys clock). But if the clock shouldn't have wrapped
                 * once then don't allow it to go backwards in time */
                ...
                /* Set the timestamp to the lowest possible value we're
                 * considering */
                hdr->timestamp += plc->ts_first_sys +
                                  plc->wrap_count * (1ull << TS_NBITS_82580);

                /* In most runs only the first if() will need evaluating - i.e
                 * our estimate is correct. */
                if (unlikely(!WITHIN_VARIANCE(cur_sys_time_ns, hdr->timestamp,
                                              MAXSKEW_82580))) {
                        /* Failed to match estimated_wraps-1 (or estimated_wraps
                         * in ==0 case) */
                        plc->wrap_count++;
                        hdr->timestamp += (1ull << TS_NBITS_82580);
                        if (!WITHIN_VARIANCE(cur_sys_time_ns, hdr->timestamp,
                                             MAXSKEW_82580)) {
                                ...
                                if (!WITHIN_VARIANCE(cur_sys_time_ns,
                                        ...
                                        if (estimated_wraps == 0) {
                                                /* 0 case Failed to match
                                                 * estimated_wraps+2 */
                                                printf("WARNING - Hardware "
                                                       "Timestamp failed to"
                                                       " match using "
                                                       "systemtime!\n");
                                                hdr->timestamp =
 ...
 */
int dpdk_read_packet_stream(libtrace_t *libtrace, dpdk_per_stream_t *stream,
                            libtrace_message_queue_t *mesg,
                            struct rte_mbuf *pkts_burst[], size_t nb_packets)
{
        size_t nb_rx; /* Number of rx packets we've received */
        while (1) {
                /* Poll for a batch of packets */
                nb_rx =
                    rte_eth_rx_burst(FORMAT(libtrace)->port, stream->queue_id,
                                     pkts_burst, nb_packets);
                if (nb_rx > 0) {
#if ENABLE_DTRACE
                        DTRACE_PROBE1(libtrace, dpdk_received_packets, nb_rx);
#endif
                        /* Got some packets - otherwise we keep spinning */
                        dpdk_ready_pkts(libtrace, stream, pkts_burst, nb_rx);
                        // fprintf(stderr, "Doing P READ PACKET port=%d q=%d\n",
                        // (int) FORMAT(libtrace)->port, (int)
                ...
                if ((nb_rx = is_halted(libtrace)) != (size_t)-1)
                         ...
                         * This relieves the pressure on memory allowing the NIC
                         * to DMA */
#if ENABLE_DTRACE
...
static int dpdk_pread_packets(libtrace_t *libtrace, libtrace_thread_t *t,
        ...
        if (nb_rx > 0) {
                for (i = 0; i < nb_rx; ++i) {
                        if (packets[i]->buffer != NULL) {
                                ...
                                if (packets[i]->buf_control !=
                                    TRACE_CTRL_PACKET) {
                                        trace_set_err(libtrace,
                                                      TRACE_ERR_BAD_PACKET,
                                                      "Expected packet buffer "
                                                      "to be empty in "
                                                      "dpdk_pread_packets()\n");
                                        return -1;
                                }
                        ...
                        packets[i]->error = 1;
                        hdr = (struct dpdk_addt_hdr *)((struct rte_mbuf *)
                                                           pkts_burst[i] +
                                                       1);
                        packets[i]->order = hdr->timestamp;
                        dpdk_prepare_packet(libtrace, packets[i], pkts_burst[i],
...
int dpdk_read_packet(libtrace_t *libtrace, libtrace_packet_t *packet)
{
        ...
        if (FORMAT(libtrace)->burst_size != FORMAT(libtrace)->burst_offset) {
                ...
                dpdk_prepare_packet(libtrace, packet, packet->buffer,
                                    packet->type, 0);
                return 1; // TODO should be bytes read, which essentially
                          // useless anyway
        }

        nb_rx = dpdk_read_packet_stream(
            libtrace, stream, NULL, FORMAT(libtrace)->burst_pkts, BURST_SIZE);
...
dpdk_get_link_type(const libtrace_packet_t *packet UNUSED)
{
...
static libtrace_direction_t dpdk_set_direction(libtrace_packet_t *packet,
...
void dpdk_get_stats(libtrace_t *trace, libtrace_stat_t *stats)
{
        ...
        if (trace->format_data == NULL ||
            FORMAT(trace)->port == RTE_MAX_ETHPORTS)
                return;
                                     ...
                                     libtrace_packet_t *packet)
{
        ...
        do {
                ...
                if (FORMAT(trace)->burst_size != FORMAT(trace)->burst_offset) {
                        ...
                        packet->buffer =
                            FORMAT(trace)
                                ->burst_pkts[FORMAT(trace)->burst_offset++];
                        dpdk_prepare_packet(trace, packet, packet->buffer,
                                            packet->type, 0);
                        event.size = 1; // TODO should be bytes read, which
                                        // essentially useless anyway

                        /* XXX - Check this passes the filter trace_read_packet
                         * normally does this for us but this wont */
                        if (trace->filter) {
                                if (!trace_apply_filter(trace->filter,
                                                        packet)) {
                                        /* Failed the filter so we loop for
                                         * another packet */
                        ...
                        /* We only want to sleep for a very short time - we are
                         * non-blocking */
...
static void dpdk_help(void)
{
        ...
        printf("\t By default the last CPU core is used if not otherwise "
               "specified.\n");
        printf("\t Only a single libtrace instance of dpdk can use the same "
               "CPU core.\n");
        printf("\t Support for multiple simultaneous instances of dpdk format "
...
static void dpdk_vdev_help(void)
{
        ...
        printf("\tThe -<coreid> is optional\n");
        printf("\tEverything before the '-' is given to DPDK in a --vdev "
               "argument\n");
