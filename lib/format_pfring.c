...
static inline int pfringzc_init_queues(const char *uridata, char *err,
                                       int errlen,
                                       ...
                                       pfring_zc_queue_mode queue_mode)
{
    ...
    for (i = 0; i < threads; i++) {
        for (j = 0; j < PFRINGZC_BATCHSIZE; j++) {
            fdata->perthreads[i].buffers[j] =
                pfring_zc_get_packet_handle(fdata->cluster);
            if (fdata->perthreads[i].buffers[j] == NULL) {
                snprintf(err, errlen,
                         "Failed to create pfringzc packet handle");
                return -1;
            }
        }

        snprintf(devname, sizeof(devname), "%s@%d", uridata, i);
        fdata->devices[i] = pfring_zc_open_device(
            fdata->cluster, devname, queue_mode,
            PF_RING_ZC_DEVICE_SW_TIMESTAMP | PF_RING_ZC_DEVICE_HW_TIMESTAMP |
                PF_RING_ZC_DEVICE_NOT_REPROGRAM_RSS);
        if (fdata->devices[i] == NULL) {
            snprintf(err, errlen, "Failed to open pfringzc device: %s",
                     devname);
            return -1;
        }
        ...
        if (fdata->bpffilter != NULL) {
            if (pfring_zc_set_bpf_filter(fdata->devices[i], fdata->bpffilter) !=
                0) {
                snprintf(err, errlen, "Failed to set pfringzc BPF filter: %s",
                         fdata->bpffilter);
                return -1;
            }
        }
                                        ...
                                        char *err, int errlen)
{
    ...
    if (linux_get_nic_queues(interface) != threads) {
        if (linux_set_nic_queues(interface, threads) != threads) {
            snprintf(err, errlen,
                     "Unable to set number of NIC queues to match the "
                     "number of processing threads: %d",
                     threads);
...
static int pfringzc_start_input(libtrace_t *libtrace)
{
    ...
    if ((ZC_FORMAT_DATA->cluster = pfring_zc_create_cluster(
             ...
             PFRINGZC_MAX_CARD_RINGS * threads, pfring_zc_numa_get_cpu_node(0),
             NULL, 0)) == NULL) {
...
static int pfringzc_start_output(libtrace_out_t *libtrace)
{
    ...
    if ((ZC_FORMAT_DATA->cluster = pfring_zc_create_cluster(
             ZC_FORMAT_DATA->clusterid,
             pfringzc_max_packet_length(libtrace->uridata), 0,
             PFRINGZC_MAX_CARD_RINGS * threads, pfring_zc_numa_get_cpu_node(0),
             NULL, 0)) == NULL) {
    ...
    if (pfringzc_init_queues(libtrace->uridata, err, sizeof(err),
                             ZC_FORMAT_DATA, threads, tx_only) == -1) {
        trace_set_err_out(libtrace, errno, "%s", err);
        return -1;
    }
...
static int pfring_pstart_input(libtrace_t *libtrace)
{
    ...
    channels = pfring_open_multichannel(libtrace->uridata, FORMAT_DATA->snaplen,
                                        pfring_flags(libtrace), ring);
    if (channels <= 0) {
        trace_set_err(libtrace, errno, "pfring_open_multichannel failed: %s",
                      strerror(errno));
        return -1;
    }
    ...
    if (libtrace->perpkt_thread_count < channels) {
        fprintf(stderr,
                "WARNING: pfring interface has %u channels, "
...
static int pfring_init_input(libtrace_t *libtrace)
{
    ...
    libtrace->format_data = (struct pfring_format_data_t *)malloc(
        sizeof(struct pfring_format_data_t));
...
static int pfringzc_init_input(libtrace_t *libtrace)
{
    ...
    libtrace->format_data = (struct pfringzc_format_data_t *)malloc(
    ...
    case TRACE_OPTION_HASHER:
        ...
        case HASHER_BALANCE:
            ...
            if ((ret = linux_get_nic_flow_rule_count(
                ...
                fprintf(stderr,
                        "%d flow director rules detected on interface %s, "
                        "RSS hashing may not work correctly!\n",
                        ret, pfring_ifname_from_uridata(libtrace->uridata));
            }
            return 0;
        case HASHER_CUSTOM:
            return -1;
...
static int pfringzc_read_batch(libtrace_t *libtrace,
                               ...
                               size_t nb_packets, bool blocking)
{
    ...
    while (received < 1) {
        ...
        received = pfring_zc_recv_pkt_burst(stream->device, stream->buffers,
                                            nb_packets, 0);

        if (received < 0) {
            trace_set_err(libtrace, errno,
                          "Failed to read packet batch from pfringzc:");
            return -1;
        }
    ...
    for (int i = 0; i < received; i++) {
        ...
        hdr->ext.ts_ns = pfring_timespec_to_systime(&stream->buffers[i]->ts);
...
static int pfring_read_generic(libtrace_t *libtrace, libtrace_packet_t *packet,
                               struct pfring_per_stream_t *stream,
                               uint8_t block, libtrace_message_queue_t *queue)
{
    ...
    do {
        if ((rc = pfring_recv(stream->pd, (u_char **)&packet->payload, 0,
                              (struct pfring_pkthdr *)local, 0)) == -1) {
            trace_set_err(libtrace, errno,
                          "Failed to read packet from pfring:");
            return -1;
        }
...
static int pfringzc_read_packet(libtrace_t *libtrace, libtrace_packet_t *packet)
{
    ...
    return pfringzc_read_batch(libtrace, &packet, NULL, stream, 1, 1);
}
                                  ...
                                  size_t nb_packets)
{
    ...
    return pfringzc_read_batch(libtrace, packets, &thread->messages, stream,
                               nb_packets, 1);
}
                                 ...
                                 libtrace_packet_t *packet)
{
    ...
    struct pfringzc_per_thread *stream = &(ZC_FORMAT_DATA->perthreads[0]);
    u_char *buffer =
        pfring_zc_pkt_buff_data(stream->buffers[0], stream->device);
...
pfring_get_lt_link_type(const libtrace_packet_t *packet UNUSED)
{
...
pfring_get_direction(const libtrace_packet_t *packet)
{
...
static void pfring_get_statistics(libtrace_t *libtrace, libtrace_stat_t *stat)
{
    ...
    if (FORMAT_DATA->interface_stats.if_name[0] != 0) {
        if (linux_get_dev_statistics(
                pfring_ifname_from_uridata(libtrace->uridata), &dev_stats) ==
            0) {

            // add card drops
            stat->dropped +=
                (dev_stats.rx_drops - FORMAT_DATA->interface_stats.rx_drops);
            stat->dropped_valid = 1;

            // calculate recieved packets by the card, this includes
            // dropped packets but not errored
            stat->received = (dev_stats.rx_packets -
            ...
            // add card errors
            stat->errors =
                (dev_stats.rx_errors - FORMAT_DATA->interface_stats.rx_errors);
            stat->errors_valid = 1;
        }
    }
...
static libtrace_eventobj_t pfringzc_event(libtrace_t *libtrace,
                                          libtrace_packet_t *packet)
{
    libtrace_eventobj_t event = {0, 0, 0.0, 0};
    struct pfringzc_per_thread *stream = &(ZC_FORMAT_DATA->perthreads[0]);
...
static int pfring_pread_packets(libtrace_t *libtrace, libtrace_thread_t *t,
                                libtrace_packet_t *packets[], size_t nb_packets)
{
    ...
    int rc = 0;
    struct pfring_per_stream_t *stream =
        (struct pfring_per_stream_t *)t->format_data;
    uint8_t block = 1;

    /* Block for the first packet, then read up to nb_packets if they
     * are available. */
    do {
        rc = pfring_read_generic(libtrace, packets[readpackets], stream, block,
                                 &t->messages);
        if (rc == READ_MESSAGE) {
            if (readpackets == 0) {
                return rc;
                                   ...
                                   bool reading)
{
    ...
    if (reading) {
        ...
        if (t->type == THREAD_PERPKT) {
            t->format_data =
                libtrace_list_get_index(FORMAT_DATA->per_stream, t->perpkt_num)
                    ->data;
        ...
        if (cpus > 1) {
            ...
            if ((s = pthread_setaffinity_np(pthread_self(), sizeof(cpuset),
                                            &cpuset)) != 0) {
                trace_set_err(libtrace, errno,
                              "Warning "
...
static void pfringzc_get_stats(libtrace_t *libtrace, libtrace_stat_t *stats)
{
    int threads, i;
    ...
    if (!(ZC_FORMAT_DATA->zero_copy)) {
        ...
        if (ZC_FORMAT_DATA->interface_stats.if_name[0] != 0) {
            if (linux_get_dev_statistics(
                ...
                stats->received = (dev_stats.rx_packets -
                                   ZC_FORMAT_DATA->interface_stats.rx_packets);
...
static int pfringzc_pregister_thread(libtrace_t *libtrace, libtrace_thread_t *t,
                                     bool reading)
{
    if (reading) {
        if (t->type == THREAD_PERPKT) {
            struct pfringzc_per_thread *stream =
                &(ZC_FORMAT_DATA->perthreads[t->perpkt_num]);
