...
{
    libtrace->format_data = (struct libtrace_format_data_t *)malloc(
        sizeof(struct libtrace_format_data_t));
...
static int bpf_start_input(libtrace_t *libtrace)
{
    ...
    if (FORMATIN(libtrace)->fd == -1) {
        trace_set_err(libtrace, TRACE_ERR_INIT_FAILED, "No free bpf devices");
        return -1;
    }

    /* Check the BPF Version is ok */
    if (ioctl(FORMATIN(libtrace)->fd, BIOCVERSION, &bv) == -1) {
        trace_set_err(libtrace, errno, "Failed to read the bpf version");
    ...
    if (bv.bv_major != BPF_MAJOR_VERSION) {
        trace_set_err(libtrace, errno,
                      "Unknown kernel BPF version (%d.%d, libtrace requires at "
                      "least %d.%d)",
                      bv.bv_major, bv.bv_minor, BPF_MAJOR_VERSION,
                      BPF_MINOR_VERSION);
        close(FORMATIN(libtrace)->fd);
        return -1;
    }
    ...
    if (bv.bv_minor < BPF_MINOR_VERSION) {
        trace_set_err(
            libtrace, errno,
            "Kernel version too old (%d.%d, libtrace requires at least %d.%d)",
    ...
    if (FORMATIN(libtrace)->promisc) {
        if (ioctl(FORMATIN(libtrace)->fd, BIOCPROMISC, NULL) == -1) {
            trace_set_err(libtrace, errno, "Failed to set promisc mode");
            close(FORMATIN(libtrace)->fd);
            return -1;
        }
    }
...
static int bpf_config_input(libtrace_t *libtrace, trace_option_t option,
                            void *data)
{
    switch (option) {
                              ...
                              libtrace_rt_types_t rt_type, uint32_t flags)
{
    ...
    if (sizeof(struct BPF_TIMEVAL) != sizeof(struct lt_bpf_timeval)) {
        ...
        replace->bh_tstamp.tv_sec =
            (uint32_t)(orig.bh_tstamp.tv_sec & 0xffffffff);
        replace->bh_tstamp.tv_usec =
            (uint32_t)(orig.bh_tstamp.tv_usec & 0xffffffff);
...
static int bpf_read_packet(libtrace_t *libtrace, libtrace_packet_t *packet)
{
    ...
    if (FORMATIN(libtrace)->remaining <= 0) {
        ...
        libtrace_create_new_bucket(FORMATIN(libtrace)->bucket,
    ...
    while (FORMATIN(libtrace)->remaining <= 0) {
        ...
        ret = select(FORMATIN(libtrace)->fd + 1, &readfds, NULL, NULL, &tout);
        ...
        if (FD_ISSET(FORMATIN(libtrace)->fd, &readfds)) {
            ...
            ret = read(FORMATIN(libtrace)->fd, FORMATIN(libtrace)->buffer,
    ...
    FORMATIN(libtrace)->bufptr +=
        BPF_WORDALIGN(BPFHDR(packet)->bh_hdrlen + BPFHDR(packet)->bh_caplen);
...
bpf_get_direction(const libtrace_packet_t *packet UNUSED)
{
...
static int bpf_get_framing_length(UNUSED const libtrace_packet_t *packet)
{
...
static int bpf_get_fd(const libtrace_t *trace) { return FORMATIN(trace)->fd; }
...
void bpf_constructor() { register_format(&bpf); }
