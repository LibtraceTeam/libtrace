...
static void guess_format(libtrace_t *libtrace, const char *filename)
{
        ...
        trace_set_err(libtrace, TRACE_ERR_BAD_FORMAT,
                      "Unable to guess format (%s)", filename);
        return;
}
...
DLLEXPORT libtrace_t *trace_create(const char *uri)
{
        ...
        if (!libtrace) {
                fprintf(stderr,
                        "Unable to allocate memory in trace_create()\n");
                return NULL;
        }
                           ...
                           void *value)
{
        ...
        if (option == TRACE_OPTION_HASHER)
                return trace_set_hasher(
                    libtrace, (enum hasher_types) * ((int *)value), NULL, NULL);
...
DLLEXPORT void trace_destroy(libtrace_t *libtrace)
{
        ...
        if (libtrace->state != STATE_NEW && libtrace->first_packets.packets) {
                ...
                ASSERT_RET(pthread_spin_destroy(&libtrace->first_packets.lock),
                           == 0);
        }
        ...
        if (libtrace->state != STATE_NEW) {
                ...
                if (libtrace->hasher_thread.type == THREAD_HASHER)
                        libtrace_message_queue_destroy(
                            &libtrace->hasher_thread.messages);
                ...
                if (libtrace->reporter_thread.type == THREAD_REPORTER)
                        libtrace_message_queue_destroy(
                            &libtrace->reporter_thread.messages);

                if (libtrace->combiner.destroy && libtrace->reporter_cbs)
                        libtrace->combiner.destroy(libtrace,
                                                   &libtrace->combiner);
...
DLLEXPORT void trace_destroy_output(libtrace_out_t *libtrace)
{
        if (!libtrace) {
                fprintf(stderr,
                        "NULL trace passed to trace_destroy_output()\n");
                return;
        }
...
DLLEXPORT libtrace_packet_t *trace_copy_packet(const libtrace_packet_t *packet)
{
        ...
        memcpy(dest->payload, packet->payload,
        ...
        if (libtrace->format->read_packet) {
                ...
                do {
                        ...
                        if (libtrace->filter) {
                                 ...
                                 * packet
                                 */
                                filtret = trace_apply_filter(libtrace->filter,
                                                             packet);
                                if (filtret == -1) {
                        ...
                        if (packet->order == 0) {
                                trace_packet_set_order(
                                    packet, libtrace->sequence_number);
                        }
                         ...
                         uint32_t flags)
{
        ...
        if (!trace) {
                fprintf(stderr,
                        "NULL trace passed into trace_prepare_packet()\n");
                return TRACE_ERR_NULL_TRACE;
        }
        ...
        if (trace->format->prepare_packet) {
                return trace->format->prepare_packet(trace, packet, buffer,
                                                     rt_type, flags);
        }
                                 ...
                                 libtrace_packet_t *packet)
{
        ...
        if (!packet) {
                trace_set_err_out(
                    libtrace, TRACE_ERR_NULL_PACKET,
                    "NULL trace passed into trace_write_packet()");
                return -1;
        }
                                        ...
                                        uint32_t *remaining)
{
        ...
        if (!packet) {
                fprintf(stderr,
                        "NULL packet passed into trace_get_packet_buffer()\n");
                return NULL;
        }
...
DLLEXPORT struct timeval trace_get_timeval(const libtrace_packet_t *packet)
{
        ...
        if (packet->which_trace_start != packet->trace->startcount) {
                ...
                struct timespec ts =
                    packet->trace->format->get_timespec(packet);
                tv.tv_sec = ts.tv_sec;
...
DLLEXPORT struct timespec trace_get_timespec(const libtrace_packet_t *packet)
{
        ...
        if (packet->which_trace_start != packet->trace->startcount) {
                ...
                /* timestamp -> timeval */
                uint64_t erfts =
                    packet->trace->format->get_erf_timestamp(packet);
                ts.tv_sec = erfts >> 32;
                ...
                ts.tv_nsec =
                    (long)(((seconds - ts.tv_sec) * 1000000000) / UINT_MAX);
        } else {
                ts.tv_sec = -1;
                ts.tv_nsec = -1;
        }
...
trace_create_filter_from_bytecode(void *bf_insns, unsigned int bf_len)
{
        ...
        return NULL;
#else
        struct libtrace_filter_t *filter = (struct libtrace_filter_t *)calloc(
            1, sizeof(struct libtrace_filter_t));
                             ...
                             libtrace_linktype_t linktype)
{
        ...
        if (filter->filterstring && !filter->flag) {
                ...
                pcap = (pcap_t *)pcap_open_dead(
                    (int)libtrace_to_pcap_dlt(linktype), 1500U);
                /* build filter */
                if (!pcap) {
                        trace_set_err(packet->trace, TRACE_ERR_BAD_FILTER,
                ...
                if (pcap_compile(pcap, &filter->filter, filter->filterstring, 1,
                                 0)) {
                        trace_set_err(packet->trace, TRACE_ERR_BAD_FILTER,
...
#        endif
        ...
        if (!packet) {
                fprintf(stderr,
                        "NULL packet passed into trace_apply_filter()\n");
                return TRACE_ERR_NULL_PACKET;
        }
        if (!filter) {
                trace_set_err(packet->trace, TRACE_ERR_NULL_FILTER,
        ...
        if (libtrace_to_pcap_dlt(linktype) == TRACE_DLT_ERROR) {
                ...
                while (libtrace_to_pcap_dlt(linktype) == TRACE_DLT_ERROR) {
                        if (!demote_packet(packet_copy)) {
                                trace_set_err(
                                    packet->trace, TRACE_ERR_NO_CONVERSION,
...
#        if HAVE_LLVM
        if (!filter->jitfilter) {
                ...
                if (!filter->jitfilter)
                        ...
                        filter->jitfilter = compile_program(
                            filter->filter.bf_insns, filter->filter.bf_len);
...
trace_set_direction(libtrace_packet_t *packet, libtrace_direction_t direction)
{
        if (!packet) {
                fprintf(stderr,
                        "NULL packet passed into trace_set_direction()\n");
                                          ...
                                          size_t size)
{
        if (!packet) {
                fprintf(stderr,
                        "NULL packet passed into trace_set_capture_length()\n");
                return ~0U;
        }
...
#        error "What on earth are you running this on?"
                ...
                trace_set_err(trace, TRACE_ERR_OPTION_UNAVAIL,
...
DLLEXPORT int trace_seek_seconds(libtrace_t *trace, double seconds)
{
        if (trace->format->seek_seconds) {
                ...
                if (trace->format->seek_timeval) {
                        ...
                        tv.tv_sec = (uint32_t)seconds;
                        tv.tv_usec = (uint32_t)(
                            ((seconds - tv.tv_sec) * 1000000) / UINT_MAX);
                        return trace->format->seek_timeval(trace, tv);
                }
                if (trace->format->seek_erf) {
                        uint64_t timestamp =
                            ((uint64_t)((uint32_t)seconds) << 32) +
                            (uint64_t)((seconds - (uint32_t)seconds) *
                                       UINT_MAX);
...
DLLEXPORT char *trace_ether_ntoa(const uint8_t *addr, char *buf)
{
        ...
        if (!buf)
                buf = staticbuf;
        snprintf(buf, (size_t)18, "%02x:%02x:%02x:%02x:%02x:%02x", addr[0],
                            ...
                            uint16_t len)
{
        ...
        if (!packet) {
                fprintf(stderr,
                        "NULL packet passed into trace_contruct_packet()\n");
                return;
        }
...
uint64_t trace_get_received_packets(libtrace_t *trace)
{
        ...
        if (!trace) {
                fprintf(stderr,
                        "NULL trace passed to trace_get_received_packets()\n");
        ...
        if (trace->format->get_received_packets) {
                if ((ret = trace->format->get_received_packets(trace)) !=
                    UINT64_MAX)
                        return ret;
...
uint64_t trace_get_filtered_packets(libtrace_t *trace)
{
        if (!trace) {
                fprintf(stderr,
                        "NULL trace passed to trace_get_filtered_packets()\n");
                return UINT64_MAX;
        }
        int i = 0;
        ...
        if (trace->format->get_filtered_packets) {
                uint64_t trace_filtered =
                    trace->format->get_filtered_packets(trace);
...
uint64_t trace_get_dropped_packets(libtrace_t *trace)
{
        if (!trace) {
                fprintf(stderr,
                        "NULL trace passed into trace_get_dropped_packets()\n");
                return UINT64_MAX;
        }
        uint64_t ret;

        if (trace->format->get_dropped_packets) {
                if ((ret = trace->format->get_dropped_packets(trace)) !=
                    UINT64_MAX)
                        return ret;
        } else if (trace->format->get_statistics) {
                struct libtrace_stat_t stat;
...
libtrace_stat_t *trace_get_statistics(libtrace_t *trace, libtrace_stat_t *stat)
{
        ...
        if (!trace) {
                fprintf(stderr,
                        "NULL trace passed into trace_get_statistics()\n");
                return NULL;
        }
        if (stat == NULL) {
                if (trace->stats == NULL)
...
#        define REQUIRE(x)                                                     \
                if (f->x)                                                      \
                fprintf(stderr,                                                \
                        "%s: Non Input format shouldn't need " #x "\n",        \
                        f->name)
		REQUIRE(read_packet);
		REQUIRE(start_input);
		REQUIRE(pause_input);
...
#        define REQUIRE(x)                                                     \
                if (f->x)                                                      \
                fprintf(stderr,                                                \
                        "%s: Non Output format shouldn't need " #x "\n",       \
                        f->name)
		REQUIRE(write_packet);
		REQUIRE(start_output);
		REQUIRE(config_output);
