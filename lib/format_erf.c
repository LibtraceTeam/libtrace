...
static int libtrace_to_erf_hdr(libtrace_out_t *libtrace,
                               libtrace_packet_t *packet, dag_record_t *erf,
                               int *framinglen, int *caplen);
...
static bool erf_can_write(libtrace_packet_t *packet)
{
    ...
    if (ltype == TRACE_TYPE_PCAPNG_META || ltype == TRACE_TYPE_NONDATA) {
        ...
        while ((*firstbyte & 0x80) == 0x80) {
            ...
            if (extsize > ntohs(erfptr->rlen)) {
                trace_set_err(packet->trace, TRACE_ERR_BAD_PACKET,
                              "Extension size is greater than dag record "
                              "record length in erf_get_framing_length()");
                return -1;
            }
        }
    }
                            ...
                            void *value)
{
    ...
    case TRACE_OPTION_CONSTANT_ERF_FRAMING:
        trace_set_err(
            libtrace, TRACE_ERR_OPTION_UNAVAIL,
            "Setting constant framing length is not supported for %s:",
            libtrace->format->name);
        return -1;
...
static int rawerf_start_input(libtrace_t *libtrace)
{
    ...
    if (!libtrace->io) {
        if (errno != 0) {
            trace_set_err(libtrace, errno, "Unable to open raw ERF file %s",
                          libtrace->uridata);
        }
        return -1;
    }
...
static int erf_fast_seek_start(libtrace_t *libtrace, uint64_t erfts)
{
    ...
    do {
        ...
        wandio_read(DATA(libtrace)->seek.index, &record, sizeof(record));
...
static int erf_seek_erf(libtrace_t *libtrace, uint64_t erfts)
{
    ...
    case INDEX_NONE:
        ...
        break;
    case INDEX_UNKNOWN:
        trace_set_err(libtrace, TRACE_ERR_SEEK_ERF,
                      "Cannot seek to erf timestamp with unknown index in "
                      "erf_seek_erf()");
        return -1;
        break;
    }
                              ...
                              uint32_t flags)
{
    ...
    if (erfptr->flags.rxerror == 1) {
        ...
        packet->payload =
            ((char *)packet->buffer) + trace_get_framing_length(packet);
    }

    if (erfptr->rlen == 0) {
        trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                      "ERF packet has an invalid record "
...
static int erf_read_packet(libtrace_t *libtrace, libtrace_packet_t *packet)
{
    ...
    while (!gotpacket) {
        ...
        if (size >= LIBTRACE_PACKET_BUFSIZE) {
            trace_set_err(libtrace, TRACE_ERR_BAD_PACKET,
                          "Packet size %u larger than supported by libtrace - "
        ...
        if ((numbytes = wandio_read(libtrace->io, buffer2, (size_t)size)) !=
            (int)size) {
        ...
        if ((linktype == TRACE_RT_ERF_META && !DATA(libtrace)->discard_meta) ||
            ...
            if (erf_prepare_packet(libtrace, packet, packet->buffer, linktype,
                                   flags)) {
                return -1;
            }
        }
                               ...
                               int *framinglen, int *caplen)
{
    ...
    if (packet->type != TRACE_RT_DATA_ERF) {
        /* Populate metadata from the packet before demoting and possibly losing
         * it */
        ...
        if (dir != TRACE_DIR_UNKNOWN)
            ...
            /* Probably uneeded, the original memset would (unintentionally?)
             * set this to 1 */
...
{
    OUTPUT->file = trace_open_file_out(libtrace, OUT_OPTIONS.compress_type,
                                       OUT_OPTIONS.level, OUT_OPTIONS.fileflag);
...
static int erf_write_packet(libtrace_out_t *libtrace, libtrace_packet_t *packet)
{
    ...
    if (!OUTPUT->file) {
        trace_set_err_out(libtrace, TRACE_ERR_BAD_IO,
                          "Attempted to write ERF packets to a "
                          "closed file, must call trace_create_output() before "
                          "calling trace_write_output()");
        return -1;
    }
    ...
    if (packet->type == TRACE_RT_DATA_ERF) {
        ...
        if (libtrace_to_erf_hdr(libtrace, packet, dag_hdr, &framinglen,
                                &caplen) < 0)
            return -1;
        return erf_dump_packet(libtrace, dag_hdr, framinglen, packet->payload,
                               caplen);
    } else {
        dag_record_t erfhdr;
        if (libtrace_to_erf_hdr(libtrace, packet, &erfhdr, &framinglen,
                                &caplen) < 0)
            return -1;
        return erf_dump_packet(libtrace, &erfhdr, framinglen, packet->payload,
                               caplen);
    }
}
...
libtrace_linktype_t erf_get_link_type(const libtrace_packet_t *packet)
{
    ...
    if (type != TYPE_LEGACY) {
        ...
        if (trace_get_capture_length(packet) < 5 || packet->payload == NULL) {
            return ~0;
        }
...
static struct libtrace_eventobj_t erf_event(struct libtrace_t *libtrace,
...
{
    switch (option) {
    case (ERF_PROV_COMMENT):
        return "Comment";
    case (ERF_PROV_GEN_TIME):
        return "Time generated";
    case (ERF_PROV_FCS_LEN):
        return "FCS Length";
    case (ERF_PROV_MASK_CIDR):
        return "Subnet CIDR";
    case (ERF_PROV_NAME):
        return "Name";
    case (ERF_PROV_DESCR):
        return "Description";
    case (ERF_PROV_APP_NAME):
        return "Application Name";
    case (ERF_PROV_HOSTNAME):
        return "Hostname";
    case (ERF_PROV_OS):
        return "Operating System";
    case (ERF_PROV_MODEL):
        return "Model";
    case (ERF_PROV_FW_VERSION):
        return "Firmware Version";
    case (ERF_PROV_SERIAL_NO):
        return "Serial Number";
    case (ERF_PROV_ORG_NAME):
        return "Organisation Name";
    case (ERF_PROV_SNAPLEN):
        return "Snap length";
    case (ERF_PROV_CARD_NUM):
        return "DAG Card Number";
    case (ERF_PROV_MODULE_NUM):
        return "DAG Module Number";
    case (ERF_PROV_LOC_NAME):
        return "Capture Location";
    case (ERF_PROV_FLOW_HASH_MODE):
        return "Flow Hash Mode";
    case (ERF_PROV_FILTER):
        return "Filter";
    case (ERF_PROV_TUNNELING_MODE):
        return "Tunneling Mode";
    case (ERF_PROV_ROTFILE_NAME):
        return "Rotfile Name";
    case (ERF_PROV_LOC_DESCR):
        return "Location Description";
    case (ERF_PROV_MEM):
        return "Stream Buffer Memory";
    case (ERF_PROV_DEV_NAME):
        return "DAG Device Name";
    case (ERF_PROV_DEV_PATH):
        return "DAG Device Path";
    case (ERF_PROV_APP_VERSION):
        return "Capture Application Version";
    case (ERF_PROV_CPU):
        return "CPU";
    case (ERF_PROV_CPU_PHYS_CORES):
        return "CPU Cores";
    case (ERF_PROV_CPU_NUMA_NODES):
        return "CPU NUMA Nodes";
    case (ERF_PROV_DAG_VERSION):
        return "DAG Software Version";
    case (ERF_PROV_IF_NUM):
        return "Interface Number";
    case (ERF_PROV_IF_SPEED):
        return "Interface Speed";
    case (ERF_PROV_IF_IPV4):
        return "Interface IPv4";
    case (ERF_PROV_IF_IPV6):
        return "Interface IPv6";
    case (ERF_PROV_IF_MAC):
        return "Interface MAC";
    case (ERF_PROV_IF_SFP_TYPE):
        return "Transceiver Type";
    case (ERF_PROV_IF_LINK_STATUS):
        return "Link Status";
    case (ERF_PROV_IF_PHY_MODE):
        return "PHY Mode";
    case (ERF_PROV_IF_PORT_TYPE):
        return "Port Type";
    case (ERF_PROV_IF_RX_LATENCY):
        return "Latency";
    case (ERF_PROV_IF_RX_POWER):
        return "Optical RX Power";
    case (ERF_PROV_IF_TX_POWER):
        return "Optical TX Power";
    case (ERF_PROV_CLK_SOURCE):
        return "CLK Source";
    case (ERF_PROV_CLK_STATE):
        return "CLK State";
    case (ERF_PROV_CLK_THRESHOLD):
        return "CLK Threshold";
    case (ERF_PROV_CLK_CORRECTION):
        return "CLK Correction";
    case (ERF_PROV_CLK_FAILURES):
        return "CLK Failures";
    case (ERF_PROV_CLK_RESYNCS):
        return "CLK Resyncs";
    case (ERF_PROV_CLK_PHASE_ERROR):
        return "CLK Phase Errors";
    case (ERF_PROV_CLK_INPUT_PULSES):
        return "CLK Input Pulses";
    case (ERF_PROV_CLK_REJECTED_PULSES):
        return "CLK Rejected Pulses";
    case (ERF_PROV_CLK_PHC_INDEX):
        return "CLK PHC Index";
    case (ERF_PROV_CLK_PHC_OFFSET):
        return "CLK PHC Offset";
    case (ERF_PROV_CLK_TIMEBASE):
        return "CLK Timebase";
    case (ERF_PROV_CLK_DESCR):
        return "CLK Description";
    case (ERF_PROV_CLK_OUT_SOURCE):
        return "CLK Output Source";
    case (ERF_PROV_CLK_LINK_MODE):
        return "CLK Link Mode";
    case (ERF_PROV_PTP_DOMAIN_NUM):
        return "PTP Domain Number";
    case (ERF_PROV_PTP_STEPS_REMOVED):
        return "PTP Steps removed";
    case (ERF_PROV_CLK_PORT_PROTO):
        return "CLK Port Protocol";
    case (ERF_PROV_STREAM_NUM):
        return "Stream Number";
    case (ERF_PROV_STREAM_DROP):
        return "Stream Dropped Records";
    case (ERF_PROV_STREAM_BUF_DROP):
        return "Stream Dropped Records (Buffer Overflow)";
    default:
        return "Unknown";
    }
    return "duno";
...
/* An ERF provenance packet can contain multiple sections of the same type per
 * packet */
libtrace_meta_t *erf_get_all_meta(libtrace_packet_t *packet)
{
    ...
    while (remaining > sizeof(dag_sec_t)) {
        ...
        if (sectype == ERF_PROV_SECTION_CAPTURE ||
            ...
            if (result->num == 1) {
                ...
                result->items = realloc(
                    result->items, result->num * sizeof(libtrace_meta_item_t));
            }
            ...
            if (result->items[result->num - 1].datatype == TRACE_META_STRING) {
                ...
                ((char *)result->items[result->num - 1].data)[ntohs(sec->len)] =
                    '\0';
                ...
                if (result->items[result->num - 1].datatype ==
                    TRACE_META_UINT32) {
        ...
        if ((ntohs(sec->len) % 4) != 0) {
            tmp = ntohs(sec->len) + (4 - (ntohs(sec->len) % 4)) +
                  sizeof(dag_sec_t);
        } else {
