...
static int tzsplive_create_socket(libtrace_t *libtrace)
{
    ...
    if (setsockopt(FORMAT_DATA->socket, SOL_SOCKET, SO_REUSEADDR, &reuse,
                   sizeof(reuse)) < 0) {
    ...
    if (setsockopt(FORMAT_DATA->socket, SOL_SOCKET, SO_RCVBUF, &recvbuf,
                   sizeof(recvbuf)) < 0) {
    ...
    if (bind(FORMAT_DATA->socket, (struct sockaddr *)listenai->ai_addr,
...
static int tzsplive_create_output_socket(libtrace_out_t *libtrace)
{
    ...
    if (getaddrinfo(FORMAT_DATA_OUT->outaddr, FORMAT_DATA_OUT->outport, &hints,
                    &FORMAT_DATA_OUT->listenai) != 0) {
    ...
    FORMAT_DATA_OUT->outsocket =
        socket(FORMAT_DATA_OUT->listenai->ai_family,
    ...
    if (setsockopt(FORMAT_DATA_OUT->outsocket, SOL_SOCKET, SO_REUSEADDR, &reuse,
                   sizeof(reuse)) < 0) {
        fprintf(stderr, "Failed to configure socket for %s:%s -- %s\n",
                FORMAT_DATA_OUT->outaddr, FORMAT_DATA_OUT->outport,
                strerror(errno));
    ...
    if (setsockopt(FORMAT_DATA_OUT->outsocket, SOL_SOCKET, SO_SNDBUF, &sendbuf,
                   sizeof(sendbuf)) < 0) {
...
static int tzsplive_init_input(libtrace_t *libtrace)
{
    ...
    libtrace->format_data =
        (tzsp_format_data_t *)calloc(1, sizeof(tzsp_format_data_t));
    ...
    if (FORMAT_DATA->buf == NULL) {
        trace_set_err(
            libtrace, TRACE_ERR_OUT_OF_MEMORY,
            "Unable "
...
static uint8_t *tzsplive_get_option(const libtrace_packet_t *packet,
                                    uint8_t option)
{
    ...
    if (hdr->version != 1) {
        trace_set_err(packet->trace, TRACE_ERR_UNSUPPORTED,
                      "TZSP version %" PRIu8 " is"
                      " not supported\n",
                      hdr->version);
        return NULL;
    }
...
static uint8_t *tzsplive_get_packet_payload(const libtrace_packet_t *packet)
{
    ...
    if (ptr == NULL) {
        fprintf(stderr,
                "Invalid TZSP packet in tzsplive_get_packet_payload()\n");
        return NULL;
    }
                                     ...
                                     libtrace_packet_t *packet, int pktlen)
{
    ...
    memmove(
        (ptr + sizeof(tzsp_tagfield_t) + sizeof(timesafe) + sizeof(timesafe)),
        ptr, pktlen - sizeof(tzsp_header_t));
...
static int tzsplive_prepare_packet(libtrace_t *libtrace UNUSED,
...
static int tzsplive_read_packet(libtrace_t *libtrace, libtrace_packet_t *packet)
{
    ...
    ret = recv(FORMAT_DATA->socket, packet->buffer,
               (size_t)LIBTRACE_PACKET_BUFSIZE, MSG_DONTWAIT);
    /* Error reading */
    if (ret == -1) {
        ...
        trace_set_err(libtrace, TRACE_ERR_BAD_IO,
                      "Error receiving on socket "
                      "%d: %s",
                      FORMAT_DATA->socket, strerror(errno));
        if (FORMAT_DATA->socket >= 0) {
            close(FORMAT_DATA->socket);
            FORMAT_DATA->socket = -1;
        }
...
static int tzsplive_write_packet(libtrace_out_t *libtrace,
                                 libtrace_packet_t *packet)
{
    ...
    if (!packet) {
        trace_set_err_out(libtrace, TRACE_ERR_NULL,
    ...
    if (packet->trace->format->type == TRACE_FORMAT_TZSPLIVE) {
        ...
        offset = sizeof(tzsp_header_t) + sizeof(tzsp_tagfield_t) +
        ...
        /* send the packet */
        ret = sendto(FORMAT_DATA_OUT->outsocket, FORMAT_DATA_OUT->buf, to_send,
                     0, FORMAT_DATA_OUT->listenai->ai_addr,
        ...
        to_send = sizeof(tzsp_header_t) + sizeof(rx_frame) + sizeof(rx_val) +
                  sizeof(tag_end) + trace_get_capture_length(packet);
        ...
        memcpy(FORMAT_DATA_OUT->buf + offset, packet->payload,
...
tzsplive_get_link_type(const libtrace_packet_t *packet)
{
    ...
    case (TZSP_ENCAP_80211):
        return TRACE_TYPE_80211;
    case (TZSP_ENCAP_80211_PRISM):
        return TRACE_TYPE_80211_PRISM;
    case (TZSP_ENCAP_80211_AVS):
        return TRACE_TYPE_UNKNOWN;
    default:
        return TRACE_TYPE_UNKNOWN;
    }
...
static struct timeval tzsplive_get_timeval(const libtrace_packet_t *packet)
{
    ...
    if ((ptr = tzsplive_get_option(packet, TZSP_LIBTRACE_CUSTOM_TAG_TIMEVAL)) !=
        NULL) {
        ptr += sizeof(tzsp_tagfield_t);
    ...
    } else if ((ptr = tzsplive_get_option(packet, TZSP_TAG_TIMESTAMP)) !=
               NULL) {
...
void tzsplive_constructor(void) { register_format(&tzsplive); }
