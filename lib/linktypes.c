...
libtrace_linktype_t pcap_linktype_to_libtrace(libtrace_dlt_t linktype)
{
    switch (linktype) {
    case TRACE_DLT_RAW:
    case TRACE_DLT_LINKTYPE_RAW:
        return TRACE_TYPE_NONE;
    case TRACE_DLT_EN10MB:
        return TRACE_TYPE_ETH;
    case TRACE_DLT_IEEE802_11:
        return TRACE_TYPE_80211;
    case TRACE_DLT_LINUX_SLL:
    ...
    case TRACE_DLT_ATM_RFC1483:
        return TRACE_TYPE_LLCSNAP;
    case TRACE_DLT_PPP:
        return TRACE_TYPE_PPP;
    case TRACE_DLT_PPP_SERIAL:
        return TRACE_TYPE_POS;
    case TRACE_DLT_C_HDLC:
        return TRACE_TYPE_HDLC_POS;
    case TRACE_DLT_OPENBSD_LOOP:
        return TRACE_TYPE_OPENBSD_LOOP;
    /* Unhandled */
...
libtrace_dlt_t libtrace_to_pcap_dlt(libtrace_linktype_t type)
{
    ...
    case TRACE_TYPE_NONE:
        return TRACE_DLT_RAW;
    case TRACE_TYPE_ETH:
        return TRACE_DLT_EN10MB;
    case TRACE_TYPE_80211:
        return TRACE_DLT_IEEE802_11;
    case TRACE_TYPE_LINUX_SLL:
        return TRACE_DLT_LINUX_SLL;
    case TRACE_TYPE_PFLOG:
        return TRACE_DLT_PFLOG;
    case TRACE_TYPE_80211_RADIO:
        return TRACE_DLT_IEEE802_11_RADIO;
    case TRACE_TYPE_LLCSNAP:
        return TRACE_DLT_ATM_RFC1483;
    case TRACE_TYPE_PPP:
        return TRACE_DLT_PPP;
    case TRACE_TYPE_HDLC_POS:
        return TRACE_DLT_C_HDLC;
    /* Theres more than one type of PPP.  Who knew? */
    case TRACE_TYPE_POS:
        return TRACE_DLT_PPP_SERIAL;
    case TRACE_TYPE_OPENBSD_LOOP:
        return TRACE_DLT_OPENBSD_LOOP;
...
libtrace_dlt_t rt_to_pcap_linktype(libtrace_rt_types_t rt_type)
{
    ...
    } else if (rt_type >= TRACE_RT_DATA_BPF &&
               rt_type < TRACE_RT_DATA_BPF_END) {
        /* RT type is in the bpf range */
        return rt_type - TRACE_RT_DATA_BPF;
    } else if (rt_type >= TRACE_RT_DATA_PCAPNG &&
               rt_type < TRACE_RT_DATA_PCAPNG_END) {
...
uint8_t libtrace_to_tzsp_type(libtrace_linktype_t linktype)
{
    switch (linktype) {
    case TRACE_TYPE_ETH:
        return TZSP_ENCAP_ETHERNET;
    case TRACE_TYPE_PPP:
        return TZSP_ENCAP_PPP;
    case TRACE_TYPE_NONE:
        return TZSP_ENCAP_RAW;
    case TRACE_TYPE_80211:
        return TZSP_ENCAP_80211;
    case TRACE_TYPE_80211_PRISM:
        return TZSP_ENCAP_80211_PRISM;
    case TRACE_TYPE_UNKNOWN:
    case TRACE_TYPE_HDLC_POS:
    case TRACE_TYPE_ATM:
    case TRACE_TYPE_LINUX_SLL:
...
unsigned int libtrace_to_arphrd_type(libtrace_linktype_t linktype)
{
    switch (linktype) {
    case TRACE_TYPE_ETH:
        return LIBTRACE_ARPHRD_ETHER;
    case TRACE_TYPE_80211:
        return LIBTRACE_ARPHRD_IEEE80211;
    case TRACE_TYPE_80211_RADIO:
        return LIBTRACE_ARPHRD_IEEE80211_RADIOTAP;
    default:
        break;
    }
    return ~0U;
...
void promote_packet(libtrace_packet_t *packet)
{
    if (packet->trace->format->type == TRACE_FORMAT_PCAP) {
        ...
        tmpbuffer =
            (char *)malloc(sizeof(libtrace_sll_header_t) + caplen + framelen);

        hdr = (libtrace_sll_header_t *)((char *)tmpbuffer + framelen);
        ...
        memcpy(tmpbuffer + sizeof(libtrace_sll_header_t) + framelen,
               packet->payload, trace_get_capture_length(packet));
...
static void replacement_pcap_header(libtrace_packet_t *packet, char *payload,
                                    int payloadlen, struct timeval *tv,
                                    int wirelen, libtrace_dlt_t linktype)
{
    ...
    memcpy(tmp + sizeof(libtrace_pcapfile_pkt_hdr_t), payload,
           (size_t)payloadlen);
...
bool demote_packet(libtrace_packet_t *packet)
{
    ...
    case TRACE_TYPE_ATM:
        ...
        payload =
            trace_get_payload_from_atm(packet->payload, &type, &remaining);
        if (payload == NULL)
            return false;
        tv = trace_get_timeval(packet);

        replacement_pcap_header(
            packet, payload, remaining, &tv,
            trace_get_wire_length(packet) -
                (trace_get_capture_length(packet) - remaining),
            TRACE_DLT_ATM_RFC1483);
    ...
    case TRACE_TYPE_ETSILI:
        ...
        remaining = trace_get_capture_length(packet);
        payload =
            trace_get_payload_from_meta(packet->payload, &ltype, &remaining);
        if (payload == NULL || remaining == 0) {
            return false;
        }
        tv = trace_get_timeval(packet);
        replacement_pcap_header(packet, payload, remaining, &tv, remaining,
                                TRACE_DLT_RAW);
        trace_clear_cache(packet);
        packet->trace = trace;
        return true;
    ...
    case TRACE_TYPE_LINUX_SLL:
        ...
        packet->payload =
            (void *)((char *)packet->payload + sizeof(libtrace_sll_header_t));
        ...
        if (packet->trace->format->type == TRACE_FORMAT_PCAP) {
            ...
            trace_set_capture_length(packet, trace_get_capture_length(packet) -
                                                 sizeof(libtrace_sll_header_t));
        }
