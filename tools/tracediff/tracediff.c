...
static void per_packet(libtrace_packet_t *a, libtrace_packet_t *b,
                       libtrace_out_t *output, libtrace_out_t *output2)
{
	char *buf_a, *buf_b;
	...
	if (!buf_a || !buf_b) {
		dump_packet(output, a);
		dump_packet(output2, b);
		dumped_diff ++;
		return;
	}
...
static void usage(char *prog) {
	...
	printf("\t-a <traceA-diff> Write traceA differences to file\n");
        printf("\t-b <traceB-diff> Write traceB differences to file\n");
	return;
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		int c = getopt_long(argc, argv, "m:a:b:", long_options, &option_index);
	...
	if (output_file[0] != NULL) {
        ...
        if (output_file[1] != NULL) {
                ...
                if (trace_is_err_output(output[1])) {
                        trace_perror_output(output[1], "Createing trace file");
                        return -1;
                }
	...
	while (trace_read_packet(trace[0], packet[0]) > 0 &&
		...
		per_packet(packet[0], packet[1], output[0], output[1]);
