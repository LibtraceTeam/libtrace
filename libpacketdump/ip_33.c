...
static char *dccp_types[] = {
    "DCCP-Request packet", "DCCP-Response packet", "DCCP-Data packet",
    "DCCP-Ack packet",     "DCCP-DataAck packet",  "DCCP-CloseReq packet",
    "DCCP-Close packet",   "DCCP-Reset packet",    "DCCP-Move packet",
};
...
DLLEXPORT void decode(int link_type UNUSED, const char *packet, unsigned len)
{
        ...
        if (htons(dccp->source) < htons(dccp->dest))
                decode_next(packet + dccp->doff * 4, len - dccp->doff * 4,
                            "dccp", htons(dccp->source));
        else
                decode_next(packet + dccp->doff * 4, len - dccp->doff * 4,
                            "dccp", htons(dccp->dest));
        return;
}
